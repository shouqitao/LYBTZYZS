...
#### 任务分解与跟踪原则（模块化功能清单 + 实时追踪）

1.  使用“模块化功能清单”作为唯一清单：Issue 创建后，AI 必须按模块生成“功能清单”，并固定在 Issue 顶部的“AI 追踪区块”。模块示例：Server(WebAPI)、Client(Desktop)、Auth、Users、Patients、MedicalCase、Consultation、Prescriptions、Herbs、Formula、Shared、Infra/CI、Docs。
2.  条目规范（最小闭环）：每条以动词开头，并包含“产出物路径/接口 + 验收点”；编号使用模块前缀，例如 [SRV-1]、[CLI-1]、[DOC-1]，跨模块依赖用 Dep: [前置编号] 标注。
3.  关联 PR 与条目：每个 PR 标题或描述必须引用对应编号（如 “feat(server): set default port 5001 [SRV-1]”），以便 AI/CI 自动勾选清单项并回填验证证据（命令输出/截图/日志）。
4.  AI 的执行逻辑：
    a. 读取并解析模块化功能清单；
    b. 仅对未勾选条目执行实现/修改；
    c. 对范围变化，生成 v2 清单并附变更摘要；
    d. 在 PR 合并后，自动勾选条目并回填“证据链接”。
5.  清单收敛：模块内条目控制在 3–8 条，过多则分期（Phase n）。

6.  编译守门（每个 Issue 的必备验收）：
    - 任何 Issue 在宣告完成前，必须确保“当前解决方案可顺利编译”。
    - 最低要求：`dotnet build LYBT.All.sln` 或按仓库提示分别构建 `LYBT.Server.sln`、`LYBT.Desktop.sln`（若 Desktop 尚处修复阶段，则至少保证受影响解决方案无编译错误）。
    - PR 描述需附“编译验证命令与结果摘要”，AI/CI 在合并前进行自动校验；不满足则不得勾选功能清单条目为完成。
...
### 原则二点五：以完成任务为导向的“够用即好”实现

为确保效率与落地质量，AI 在“分析 Issue 需求并创建实现方案”阶段，必须遵循以下准则：
...
5.  推荐窗口：只有在“代码分析阶段”（对现有实现进行评估与审查时），才主动给出重构建议、替代技术或更优方案，并明确标注为“建议/可选”。
