...
在处理缺陷或技术债务时，应先从架构与分层角度评估影响，避免“为修复而修复”的临时补丁。

- **统一标准优先**：所有改动必须遵循既有文档与架构约束；若需扩展标准，先在 tech-design/ADR 中补充依据，再实施。
- **分层清晰**：明确当前修改所在层级（Controller / Service / Module / Shared），保持职责边界，杜绝越层调用或投机捷径。
- **简化而非堆叠**：优先通过删除冗余、统一配置、精简层次解决问题，禁止以新增粘合层掩盖设计缺陷。
- **跨模块影响**：变更前评估是否波及其他模块或共享契约，必要时同步更新对应模块与文档，维持系统一致。
- **记录取舍**：任何偏离既有架构的修复都需在 Issue/PR 中说明原因、风险与回滚方案，确保决策可追溯。
