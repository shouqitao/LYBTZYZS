...
namespace LYBT.Tests.Desktop.EndToEnd.MedicalCase;
...
/// 夹具: DesktopFixture（轻量级 DI 容器，不依赖 ViewModel/Prism）
/// </summary>
public class MedicalCaseAggregateE2ETests : IClassFixture<DesktopFixture>
{
    private readonly DesktopFixture _fixture;

    public MedicalCaseAggregateE2ETests(DesktopFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task MultipleMedicalCases_SamePatient_IndependentAggregates()
    {
        ...
        // Act
        var created1 = await ds.CreateAsync(mc1);
        // 业务规则: 同一患者不能同时有两个进行中/暂存的医案
        // 需要先完成第一个医案，才能创建第二个
        await ds.CompleteAsync(created1.Id);
        var created2 = await ds.CreateAsync(mc2);
