...
namespace LYBT.Tests.Desktop.EndToEnd.LocalMode;
...
public class DataSourceIntegrationTests : IClassFixture<DesktopFixture>
{
    ...
    [Fact]
    public void DI_AllDataSources_CanBeResolved()
    {
        // Arrange
    ...
    public async Task MultipleDataSources_SameProvider_ShareDatabase()
    {
        ...
        var scopedDb = scope.ServiceProvider.GetRequiredService<LocalDbContext>();
        var dbPatient = await scopedDb.Patients.FindAsync(patient.Id);
        var dbHerb = await scopedDb.Herbs.FindAsync(herb.Id);
