...
/// 否则任何触碰加密列的读写都会以「敏感数据加密/解密失败」（ERR-00013 / HTTP 422）失败
/// ——这是本地模式 E2E 大批失败的根因（患者/挂号/医案/处方链路全部经患者表）。</para>
/// <para>注入方式与生产部署一致（环境变量）；密钥值与 <c>AesGcmValueConverter</c> 的 Development 回退测试密钥同值，
/// 使开发机既有密文数据在测试库中仍可读。已由外部显式提供密钥时（如 CI 注入真实密钥）不覆盖。</para>
/// </remarks>
