...
public class AuthenticationServiceTests
{
    ...
    #region LoginWithAutoTokenAsync (DIV-D05)

    /// <summary>
    /// 测试：AutoLoginToken 登录成功
    /// </summary>
    [Fact]
    public async Task LoginWithAutoTokenAsync_Success_ReturnsLoginResponse()
    {
        ...
        {
            UserName = "test_user",
            AutoLoginToken = "valid_auto_token"
        };
        ...
        // Assert
        result.Should().NotBeNull();
        result.IsSuccess.Should().BeTrue("有效的AutoLoginToken应登录成功");
        result.Data.Should().NotBeNull();
    ...
    /// <summary>
    /// 测试：AutoLoginToken 无效/过期时登录失败
    /// </summary>
    [Fact]
    public async Task LoginWithAutoTokenAsync_InvalidToken_ReturnsFailure()
    {
        // Arrange
        var request = new AutoLoginRequest
        {
            UserName = "test_user",
            AutoLoginToken = "expired_auto_token"
        };
        ...
        _authApi.LoginWithAutoTokenAsync(request)
            ...
            {
                Success = false,
                Message = "AutoLoginToken已过期或已被撤销"
            }));
        ...
        // Assert
        result.Should().NotBeNull();
        result.IsSuccess.Should().BeFalse("过期的AutoLoginToken应登录失败");
        result.Message.Should().Contain("AutoLoginToken已过期或已被撤销");
    }

    /// <summary>
    /// 测试：AutoLoginToken 登录时网络异常
    /// </summary>
    [Fact]
    public async Task LoginWithAutoTokenAsync_NetworkException_ReturnsFailure()
    {
        // Arrange
        var request = new AutoLoginRequest
        {
        ...
        _authApi.LoginWithAutoTokenAsync(request)
            .Returns(Task.FromException<ApiResponse<LoginResponse>>(
        ...
        // Assert
        result.Should().NotBeNull();
        result.IsSuccess.Should().BeFalse("网络异常时应返回失败");
