...
/// <summary>
/// Gitee Releases 更新源单测（US-SHELL-010/012：公网分发走 Gitee Releases）。
/// </summary>
/// <remarks>
/// <para>锁定的运行时契约（由 Velopack 1.2.0 的 GitBase 行为决定，非本仓约定）：</para>
/// <list type="number">
///   <item>API 基地址为 <c>{host}/api/v5/</c>（<c>GiteaSource</c> 默认 <c>/api/v1/</c>，Gitee 不同）。</item>
...
/// 全程不触网。</para>
/// </remarks>
public class GiteeReleaseSourceTests
{
    private const string RepoUrl = "https://gitee.com/owner/repo";
    ...
    /// <summary>Gitee OpenAPI v5 的 releases 列表响应样本（字段与 GitHub/Gitea 同构）。</summary>
    ...
    private static (GiteeReleaseSource Source, List<string> RequestedUrls) CreateSource(
        Func<string, string?> respond, bool prerelease = false, string? accessToken = null)
    {
        ...
        return (new GiteeReleaseSource(RepoUrl, accessToken, prerelease, downloader), requested);
    }
    ...
    [Fact]
    public async Task GetReleaseFeed_UsesGiteeApiV5BaseUrl()
    {
        ...
        requested[0].Should().StartWith("https://gitee.com/api/v5/repos/",
            "Gitee 的 API 根是 /api/v5/（GiteaSource 默认 /api/v1/ 会 404）");
    }
