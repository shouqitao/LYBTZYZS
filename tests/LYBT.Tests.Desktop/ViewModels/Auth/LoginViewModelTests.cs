...
namespace LYBT.Tests.Desktop.ViewModels.Auth;
...
///
/// DIV-D04: 使用 UsernameChangeLogicTester 而非真实 LoginViewModel 进行测试。
/// 原因: LoginViewModel 构造函数内部有 Task.Run 调用 LoadSavedCredentialsAsync，
/// 该方法依赖 Application.Current?.Dispatcher 获取 WPF 线程，在单元测试环境中为 null。
/// _savedUsername 是 private 字段，仅在 LoadSavedCredentialsAsync 中赋值，无法从外部设置。
/// 因此使用独立的逻辑测试类，其清空逻辑与 LoginViewModel.Username setter 完全一致:
///   shouldClearPassword = _savedUsername != null && !string.IsNullOrEmpty(_savedUsername)
///     && !string.IsNullOrEmpty(value) && value != _savedUsername && !string.IsNullOrEmpty(_password)
/// </summary>
public class LoginViewModelUsernameChangeTests
{
    /// <summary>
    /// 测试辅助类：模拟LoginViewModel的用户名变更逻辑
    /// 由于LoginViewModel构造函数依赖 WPF Application.Current，使用简化版本进行单元测试
    /// 逻辑与 LoginViewModel.Username setter (line 45-46) 保持一致
    /// </summary>
