...
namespace LYBT.Tests.Desktop.ViewModels.Shell.Login;
...
/// <summary>
/// LoginCoordinator 本地模式登录测试
/// DIV-D01: Local mode login path coverage
/// </summary>
public class LoginCoordinatorLocalModeTests
{
    private readonly ILogger<LoginCoordinator> _logger;
    private readonly IAuthenticationService _authService;
    private readonly ITokenStorageService _tokenStorage;
    ...
    public LoginCoordinatorLocalModeTests()
    {
        _logger = Substitute.For<ILogger<LoginCoordinator>>();
        _authService = Substitute.For<IAuthenticationService>();
        _tokenStorage = Substitute.For<ITokenStorageService>();
        ...
        _navigationCoordinator = Substitute.For<INavigationCoordinator>();
        _stateMachine = Substitute.For<IAuthenticationStateMachine>();
        _localAuthService = Substitute.For<ILocalAuthService>();

        // 配置 ConnectionMode = Local
        _configuration = Substitute.For<IConfiguration>();
        _configuration["ConnectionMode"].Returns("Local");
        ...
        _stateMachine.Fire(Arg.Any<AuthEvent>(), Arg.Any<string?>())
            .Returns(callInfo =>
            {
                ...
                if (StateTransitions.TryGetValue((_currentMockState, evt), out var newState))
                {
                    ...
                    _currentMockState = newState;
                    _stateMachine.StateChanged += Raise.EventWith(
                        new AuthStateChangedEventArgs(previousState, newState, evt, callInfo.ArgAt<string?>(1)));
                    return true;
                }
                return false;
            });
    ...
    [Fact]
    public async Task LoginAsync_LocalMode_Success_ShouldTransitionToAuthenticated()
    {
        // Arrange
        ...
        // Act
        var result = await _sut.LoginAsync("localuser", "password");

        // Assert
        result.Success.Should().BeTrue();
        result.User.Should().NotBeNull();
        result.User!.UserName.Should().Be("localuser");
        _sut.CurrentState.Should().Be(AuthState.Authenticated);
        _sut.IsLoggedIn.Should().BeTrue();
    }

    [Fact]
    public async Task LoginAsync_LocalMode_InvalidCredentials_ShouldReturnFailed()
    {
        // Arrange
        _localAuthService.ValidateAsync("localuser", "wrong", Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<UserDetailDto?>(null));

        // Act
        var result = await _sut.LoginAsync("localuser", "wrong");
    ...
    [Fact]
    public async Task LoginAsync_LocalMode_WithoutLocalAuthService_ShouldReturnFailed()
    {
        // Arrange - 不注入 ILocalAuthService 的 LoginCoordinator
        var sutWithoutLocalAuth = new LoginCoordinator(
            _logger,
            _authService,
            _tokenStorage,
            _sessionManager,
        ...
        // Act
        var result = await sutWithoutLocalAuth.LoginAsync("localuser", "password");

        // Assert
        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("本地认证服务未配置");
    }
}
