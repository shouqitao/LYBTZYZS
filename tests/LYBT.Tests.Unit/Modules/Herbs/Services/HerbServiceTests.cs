...
namespace LYBT.Tests.Unit.Modules.Herbs.Services;
...
public class HerbServiceTests : IDisposable
{
    ...
    public async Task GetPagedAsync_WithDefaultParameters_ShouldReturnPagedResult()
    {
        ...
        _repositoryMock
            .GetPagedAsync(1, 20, Arg.Any<string?>(), Arg.Any<string?>())
            .Returns(pagedResult);
    ...
    public async Task GetPagedAsync_WithKeyword_ShouldFilterByNameOrPinyin()
    {
        ...
        _repositoryMock
            .GetPagedAsync(1, 20, "黄芪", Arg.Any<string?>())
            .Returns(pagedResult);
    ...
    public async Task GetPagedAsync_WithCategory_ShouldFilterByCategory()
    {
        ...
        _repositoryMock
            .GetPagedAsync(1, 20, Arg.Any<string?>(), "补气药")
            .Returns(pagedResult);
    ...
    public async Task GetPagedAsync_WithPagination_ShouldReturnCorrectPage()
    {
        ...
        _repositoryMock
            .GetPagedAsync(2, 3, Arg.Any<string?>(), Arg.Any<string?>())
            .Returns(pagedResult);
    ...
    public async Task GetPagedAsync_WithNoResults_ShouldReturnEmptyList()
    {
        ...
        _repositoryMock
            .GetPagedAsync(1, 20, "不存在", Arg.Any<string?>())
            .Returns(pagedResult);
    ...
    [Fact]
    public async Task BatchImportAsync_WithNewHerbs_ShouldDelegateToImportExportService()
    {
        // Arrange
        ...
        var importResult = new HerbBatchImportResultDto
        {
            SuccessCount = 2,
            FailureCount = 0
        };
        ...
        _importExportMock
            .BatchImportAsync(dtos, DuplicateStrategy.Skip)
            .Returns(Result<HerbBatchImportResultDto>.Success(importResult));
        ...
        await _importExportMock.Received(1).BatchImportAsync(dtos, DuplicateStrategy.Skip);
    }

    [Fact]
    public async Task BatchImportAsync_WithDuplicates_SkipStrategy_ShouldDelegateToImportExportService()
    {
        // Arrange
        var dtos = new List<HerbInputDto>
        {
    ...
    [Fact]
    public async Task BatchImportAsync_WithDuplicates_ErrorStrategy_ShouldDelegateToImportExportService()
    {
        // Arrange
        var dtos = new List<HerbInputDto>
        {
        ...
        {
            FailureCount = 1,
            Failures = new List<HerbImportFailureDto>
            {
                new HerbImportFailureDto { Reason = "药材名称重复" }
            }
        };

        _importExportMock
            .BatchImportAsync(dtos, DuplicateStrategy.Error)
            .Returns(Result<HerbBatchImportResultDto>.Success(importResult));
    ...
    [Fact]
    public async Task BatchImportAsync_WithOverLimit_ShouldDelegateToImportExportService()
    {
        // Arrange
        ...
        _importExportMock
            .BatchImportAsync(dtos, DuplicateStrategy.Skip)
            .Returns(Result<HerbBatchImportResultDto>.Failure("批量导入最多支持10000条记录"));
    ...
    [Fact]
    public async Task GetAllForExportAsync_ShouldDelegateToImportExportService()
    {
        // Arrange
        var herbDtos = new List<HerbDetailDto>
        {
            new HerbDetailDto { Name = "药材1", Category = "补气药" },
            new HerbDetailDto { Name = "药材2", Category = "补血药" },
            new HerbDetailDto { Name = "药材3", Category = "清热药" }
        };

        _importExportMock
            .GetAllForExportAsync(Arg.Any<string?>())
            .Returns(Result<List<HerbDetailDto>>.Success(herbDtos));
        ...
        await _importExportMock.Received(1).GetAllForExportAsync(null);
    }

    [Fact]
    public async Task GetAllForExportAsync_WithCategory_ShouldDelegateWithCategory()
    {
        // Arrange
        var herbDtos = new List<HerbDetailDto>
        {
            new HerbDetailDto { Name = "黄芪", Category = "补气药" }
        };

        _importExportMock
            .GetAllForExportAsync("补气药")
            .Returns(Result<List<HerbDetailDto>>.Success(herbDtos));
        ...
        await _importExportMock.Received(1).GetAllForExportAsync("补气药");
    }
    ...
    [Fact]
    public void GenerateImportTemplate_ShouldDelegateToImportExportService()
    {
        // Arrange
        var templateStream = new MemoryStream(new byte[] { 0x50, 0x4B, 0x03, 0x04 }); // mock xlsx header
        ...
        _importExportMock.Received(1).GenerateImportTemplate();
    }
