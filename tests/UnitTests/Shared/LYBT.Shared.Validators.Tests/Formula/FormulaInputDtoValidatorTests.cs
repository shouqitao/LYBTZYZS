...
namespace LYBT.Shared.Validators.Tests.Formula;
...
public class FormulaInputDtoValidatorTests
{
    ...
    public void Validate_WithEffectTooLong_ShouldFail()
    {
        // Arrange
        var dto = CreateValidFormulaInputDto();
        dto.Effect = new string('补', 201); // > 200
        ...
        result.ShouldHaveValidationErrorFor(x => x.Effect)
            .WithErrorMessage("功效长度不能超过200个字符");
    }
    ...
    [Fact]
    public void Validate_WithNullHerbs_ShouldPass()
    {
        // Arrange - Herbs为null时跳过验证（When条件）
        var dto = CreateValidFormulaInputDto();
        dto.Herbs = null!;
