...
public class TokenRevocationServiceTests : IDisposable
{
    ...
    #region RevokeAllUserTokensAsync 测试

    [Fact]
    public async Task RevokeAllUserTokensAsync_WithMultipleTokens_ShouldRevokeAll()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var userType = "user";
        var reason = "Security issue";
        ...
        // Act
        var result = await _sut.RevokeAllUserTokensAsync(userId, userType, reason);
        ...
        revokedTokens.Should().AllSatisfy(t =>
        {
        ...
        var auditLog = await _context.SecurityAuditLogs
            .FirstOrDefaultAsync(l => l.EventType == "TokenRevoked" && l.UserId == userId);
        auditLog.Should().NotBeNull();
        auditLog!.Metadata.Should().Contain("\"revokedCount\":3");
    }
    ...
    [Fact]
    public async Task RevokeAllUserTokensAsync_ShouldOnlyRevokeSpecifiedUserType()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var reason = "Test";
        ...
        await _context.RefreshTokens.AddRangeAsync(userToken, adminToken);
        await _context.SaveChangesAsync();

        // Act - 只撤销user类型
        var result = await _sut.RevokeAllUserTokensAsync(userId, "user", reason);

        // Assert
        result.Should().Be(1);

        var userTokenAfter = await _context.RefreshTokens.FirstAsync(t => t.Token == "user-token");
        userTokenAfter.IsRevoked.Should().BeTrue();

        var adminTokenAfter = await _context.RefreshTokens.FirstAsync(t => t.Token == "admin-token");
        adminTokenAfter.IsRevoked.Should().BeFalse();
    }
