...
namespace LYBT.Server.Tests.Common.TestConfiguration;
...
/// 配置构建器扩展
/// </summary>
public static class InMemoryConfigurationBuilder
{
    /// <summary>
    /// 创建测试用的内存配置
    /// </summary>
    public static IConfiguration CreateTestConfiguration(Action<Dictionary<string, string>>? configure = null)
    {
        var configData = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

        // 设置默认的测试配置
        SetDefaultTestConfiguration(configData);

        // 允许自定义配置
        configure?.Invoke(configData);
    ...
    /// 设置默认测试配置
    /// </summary>
    private static void SetDefaultTestConfiguration(Dictionary<string, string> configData)
    {
        // JWT配置
        configData["Lybt:Jwt:SecretKey"] = "ThisIsAVeryStrongSecretKeyForTesting123456789012345678901234567890";
        configData["Lybt:Jwt:Issuer"] = "LYBT-Test";
        configData["Lybt:Jwt:Audience"] = "LYBT-TestUsers";
        configData["Lybt:Jwt:AccessTokenExpirationMinutes"] = "30";
        configData["Lybt:Jwt:RefreshTokenExpirationDays"] = "7";

        // 数据库配置
        configData["ConnectionStrings:DefaultConnection"] = "Server=(localdb)\\mssqllocaldb;Database=LYBT_Test;Trusted_Connection=true;";

        // 日志配置
        configData["Logging:LogLevel:Default"] = "Warning";
        configData["Logging:LogLevel:Microsoft.AspNetCore"] = "Warning";

        // API配置
        configData["Lybt:Api:Version"] = "v1";
        configData["Lybt:Api:BasePath"] = "/api";
        configData["Lybt:Api:Cors:AllowedOrigins"] = "http://localhost:3000;http://localhost:5000";
    }
}