...
public class PrescriptionEditFlowTests
{
    ...
    public void PrescriptionEditFlow_AddMultipleHerbs_ShouldCalculateTotalCorrectly()
    {
        ...
        // Assert - 验证各项小计
        item1.ItemTotal.Should().Be(5m, "当归 10g x 0.5元/g = 5元");
        item2.ItemTotal.Should().Be(12m, "黄芪 15g x 0.8元/g = 12元");
        item3.ItemTotal.Should().Be(1.2m, "甘草 6g x 0.2元/g = 1.2元");

        // Assert - 验证总价
        var totalPrice = herbItems.Sum(h => h.ItemTotal);
    ...
    public void PrescriptionEditFlow_ModifyDosage_ShouldUpdatePriceAutomatically()
    {
        ...
        item.ItemTotal.Should().Be(10m, "修改后小计应为 20g x 0.5元/g = 10元");
    }
    ...
    public void PrescriptionEditFlow_ChangeHerb_ShouldUpdatePriceAutomatically()
    {
        ...
        item.ItemTotal.Should().Be(8m, "黄芪 10g x 0.8元/g = 8元");
    }
    ...
    /// <summary>
    /// 测试：拼音码搜索流程 - 验证搜索结果正确排序
    /// 核心功能：精确匹配的药材排在最前
    /// </summary>
    [Fact]
    public void PinyinSearchFlow_ProgressiveInput_ShouldRankExactMatchFirst()
    {
        // Arrange
        var herbCatalog = CreateHerbCatalog();
        var item = new PrescriptionHerbItem { AllHerbs = herbCatalog };
        ...
        // Assert - 验证搜索结果
        item.FilteredHerbs.Should().NotBeEmpty("输入'dg'应返回搜索结果");
        item.FilteredHerbs.Should().Contain(h => h.Name == "当归", "搜索结果应包含当归");
        item.FilteredHerbs.First().Name.Should().Be("当归", "精确匹配的当归应排在第一位");
    }
    ...
    public void ValidationFlow_InvalidDosage_ShouldShowError()
    {
        ...
        item.DosageValidationMessage.Should().Contain("500", "应提示最大剂量限制");
    }
    ...
    public void MultiItemWorkflow_BatchAdd_ShouldMaintainConsistency()
    {
        ...
        herbItems[1].ItemTotal.Should().Be(12m);
        herbItems[2].ItemTotal.Should().Be(0m, "空槽位价格应为0");
        herbItems[3].ItemTotal.Should().Be(0m, "空槽位价格应为0");

        // Assert - 计算有效药材总价
        var validItems = herbItems.Where(h => h.HerbId != Guid.Empty);
        var totalPrice = validItems.Sum(h => h.ItemTotal);
