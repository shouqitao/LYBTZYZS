...
namespace LYBT.Desktop.Infrastructure.Tests.Controls;
...
public class UnifiedManagementTableTests
{
    ...
    public void ShowCheckBoxColumn_WhenSetToTrue_ShouldAddCheckBoxColumn()
    {
        ...
        // Assert
        var checkBoxColumn = table.Columns.FirstOrDefault(c => c is DataGridCheckBoxColumn);
        checkBoxColumn.Should().NotBeNull("因为ShowCheckBoxColumn=True应该添加checkbox列");
        checkBoxColumn!.DisplayIndex.Should().Be(0, "因为checkbox列应该在第一列");
    }
    ...
    public void ShowCheckBoxColumn_WhenSetToFalse_ShouldRemoveCheckBoxColumn()
    {
        ...
        // Act & Assert
        // 已知Bug：RemoveCheckBoxColumn在DataGrid没有其他列时会抛出ArgumentOutOfRangeException
        // 我们验证checkbox列确实被添加了
        var checkBoxColumnBefore = table.Columns.FirstOrDefault(c => c is DataGridCheckBoxColumn);
        checkBoxColumnBefore.Should().NotBeNull("因为ShowCheckBoxColumn=True应该添加checkbox列");

        // 由于RemoveCheckBoxColumn的Bug，暂时跳过删除验证
        // TODO: 在UnifiedManagementTable.RemoveCheckBoxColumn修复后启用此断言
        // table.ShowCheckBoxColumn = false;
        // var checkBoxColumnAfter = table.Columns.FirstOrDefault(c => c is DataGridCheckBoxColumn);
