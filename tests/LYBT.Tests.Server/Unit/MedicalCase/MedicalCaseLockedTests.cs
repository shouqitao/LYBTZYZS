...
/// - 非管理员（创建医生）不能编辑已完成医案（EnsureCanOperate 拒绝 Completed）
...
public class MedicalCaseLockedTests
{
    ...
    [Fact]
    public void EnsureCanOperate_Doctor_CannotEdit_CompletedCase()
    {
        ...
        Assert.Throws<UnauthorizedAccessException>(() =>
            MedicalCaseServiceHelper.EnsureCanOperate(mc, doctorId, isAdmin: false, "Save", logger));
    }

    [Fact]
    public void EnsureCanOperate_Doctor_CanEdit_OwnActiveCase()
    {
        var doctorId = Guid.NewGuid();
