...
namespace LYBT.Tests.Server.Sync;
...
/// </summary>
public sealed class SyncIntegrationTests : IntegrationTestBase
{
    private const string BaseUrl = "/api/v1/sync";

    public SyncIntegrationTests(ServerFixture fixture) : base(fixture) { }

    #region Helpers

    /// <summary>通过DbContext直接种子药材并返回ID</summary>
    ...
    public async Task GetEntityTypes_WithAuth_ShouldReturn3Types()
    {
        // Act
        var admin = await LoginAsAdminAsync();
        var response = await admin.GetAsync($"{BaseUrl}/entity-types");
    ...
    {
        // Act
        var response = await AnonymousClient.GetAsync($"{BaseUrl}/entity-types");
    ...
    public async Task GetEntityTypes_WithDoctorAuth_ShouldReturn200()
    {
        ...
        // Act
        var response = await doctor.GetAsync($"{BaseUrl}/entity-types");
    ...
    public async Task GetMetadata_Herb_ShouldReturnMetadataWithChecksum()
    {
        ...
        // Act
        var response = await admin.GetAsync($"{BaseUrl}/metadata?entityType=Herb");
    ...
    public async Task GetMetadata_InvalidType_ShouldReturnBusinessFail()
    {
        // Act
        var admin = await LoginAsAdminAsync();
        var response = await admin.GetAsync($"{BaseUrl}/metadata?entityType=InvalidType");
    ...
    public async Task GetMetadata_EmptyType_ShouldReturn400()
    {
        // Act
        var admin = await LoginAsAdminAsync();
        var response = await admin.GetAsync($"{BaseUrl}/metadata?entityType=");
    ...
    public async Task Compare_EmptyLocal_ShouldReturnServerOnlyDiffs()
    {
        ...
        // Act
        var response = await admin.PostAsJsonAsync($"{BaseUrl}/compare", input);
    ...
    public async Task Compare_IdenticalChecksum_ShouldNotAppearInDiffs()
    {
        ...
        var metaResponse = await admin.GetAsync($"{BaseUrl}/metadata?entityType=Herb");
    ...
    public async Task Upload_NewHerb_ShouldCreateAndReturnSuccess()
    {
        ...
        // Act
        var response = await admin.PostAsJsonAsync($"{BaseUrl}/upload", input);
        ...
        var downloadResponse = await admin.PostAsJsonAsync($"{BaseUrl}/download", downloadInput);
    ...
    public async Task Download_ExistingHerb_ShouldReturnEntityData()
    {
        ...
        // Act
        var response = await admin.PostAsJsonAsync($"{BaseUrl}/download", input);
    ...
    public async Task Delete_FormulaNoReferences_ShouldSucceed()
    {
        ...
        await admin.PostAsJsonAsync($"{BaseUrl}/upload", uploadInput);
        ...
        var response = await admin.PostAsJsonAsync($"{BaseUrl}/delete", deleteInput);
    ...
    public async Task Delete_EmptyIdList_ShouldReturn400()
    {
        ...
        // Act
        var response = await admin.PostAsJsonAsync($"{BaseUrl}/delete", input);
    ...
    public async Task FullSyncWorkflow_CompareUploadDownload_ShouldWork()
    {
        ...
        var compareResponse = await admin.PostAsJsonAsync($"{BaseUrl}/compare", compareInput);
        ...
        var uploadResponse = await admin.PostAsJsonAsync($"{BaseUrl}/upload", uploadInput);
        ...
        var finalResponse = await admin.PostAsJsonAsync($"{BaseUrl}/compare", finalCompare);
