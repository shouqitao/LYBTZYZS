...
public class EnvironmentConfigFileClosureTests : IDisposable
{
    ...
    private static void InvokeEnsure(string environment, string baseDir)
    {
        var method = typeof(Program).GetMethod("EnsureEnvironmentConfigFiles",
            BindingFlags.NonPublic | BindingFlags.Static)!;
        method.Invoke(null, new object[] { environment, baseDir });
    }

    [Fact]
    public void MissingFiles_AreGenerated_WithPlaceholders()
    {
        InvokeEnsure("Production", _tempDir);

        File.Exists(Path.Combine(_tempDir, "config", "appsettings.json")).Should().BeTrue("缺失 appsettings.json 应自动生成");
        File.Exists(Path.Combine(_tempDir, "config", "appsettings.Production.json")).Should().BeTrue("缺失环境文件应自动生成");

        var appJson = File.ReadAllText(Path.Combine(_tempDir, "config", "appsettings.json"));
        ...
        var prodJson = File.ReadAllText(Path.Combine(_tempDir, "config", "appsettings.Production.json"));
    ...
    {
        // 已存在文件不被覆盖（用户自定义配置优先）
        var customPath = Path.Combine(_tempDir, "config", "appsettings.json");
    ...
    {
        InvokeEnsure("Development", _tempDir);

        File.Exists(Path.Combine(_tempDir, "config", "appsettings.Development.json")).Should().BeTrue();
        var devJson = File.ReadAllText(Path.Combine(_tempDir, "config", "appsettings.Development.json"));
