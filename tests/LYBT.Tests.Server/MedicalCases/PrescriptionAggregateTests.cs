...
/// </summary>
public sealed class PrescriptionAggregateTests : IntegrationTestBase
{
    ...
    public PrescriptionAggregateTests(ServerFixture fixture, ITestOutputHelper output)
        : base(fixture)
    {
        _output = output;
    }
    ...
    private async Task SetPrescriptionFlagAsync(Guid medicalCaseId)
    {
        ...
        var resp = await doctor.PutAsJsonAsync(
    ...
    {
        var doctor = await LoginAsDoctorAsync();
        var resp = await doctor.GetAsync($"{BaseUrl}/{id}");
    ...
    public async Task AggregateSave_WithPrescription_ShouldPersist()
    {
        ...
        var resp = await doctor.PutAsJsonAsync($"{BaseUrl}/{medicalCase.Id}", saveInput);
    ...
    public async Task AggregateSave_ConsecutiveUpdates_ShouldNotCauseConcurrencyException()
    {
        ...
        var resp1 = await doctor.PutAsJsonAsync($"{BaseUrl}/{medicalCase.Id}", save1);
        ...
        var resp2 = await doctor.PutAsJsonAsync($"{BaseUrl}/{medicalCase.Id}", save2);
