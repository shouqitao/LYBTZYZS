...
namespace LYBT.Tests.Server.Formulas;

/// <summary>
/// 验方 Service 层集成测试 - 导入/验证工作流。
/// 测试 Service 层逻辑而非 HTTP 端点，补充 FormulaIntegrationTests 未覆盖的导入和验证流程。
/// </summary>
public sealed class FormulaServiceIntegrationTests : IntegrationTestBase
{
    public FormulaServiceIntegrationTests(ServerFixture fixture) : base(fixture) { }

    /// <summary>
    /// 种子测试用药材到数据库，返回 (Id, Name) 列表。
