...
namespace LYBT.Tests.Server.Diagnostics;

/// <summary>
/// DiagnosticsController集成测试
/// 测试日志级别管理API
/// </summary>
public sealed class DiagnosticsControllerIntegrationTests : IntegrationTestBase
{
    private readonly ITestOutputHelper _output;

    public DiagnosticsControllerIntegrationTests(ServerFixture fixture, ITestOutputHelper output) : base(fixture)
    {
        _output = output;
    }
    ...
    public async Task GetLoggingStatus_ShouldReturnCurrentStatus()
    {
        ...
        // Act
        var response = await sysAdmin.GetAsync("/api/v1/diagnostics/logging/status");
    ...
    public async Task EnableDebugMode_WithValidRequest_ShouldSucceed()
    {
        ...
        // Act
        var response = await sysAdmin.PostAsJsonAsync("/api/v1/diagnostics/logging/debug/enable", request);
    ...
    public async Task DisableDebugMode_ShouldRestoreDefaultLevel()
    {
        ...
        // Act
        var response = await sysAdmin.PostAsync("/api/v1/diagnostics/logging/debug/disable", null);
    ...
    public async Task SetLoggingLevel_WithValidLevel_ShouldSucceed()
    {
        ...
        // Act
        var response = await sysAdmin.PostAsJsonAsync("/api/v1/diagnostics/logging/level", request);
