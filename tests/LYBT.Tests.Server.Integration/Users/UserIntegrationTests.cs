...
namespace LYBT.Tests.Server.Integration.Users;
...
public class UserIntegrationTests
{
    ...
    public UserIntegrationTests(WebApiFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task CreateUser_DuplicateUsername_ShouldNotCreateDuplicate()
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            var listResponse = await _fixture.AdminClient
    ...
    {
        // Arrange & Act - Fixture已种子admin+doctor共2个用户
        var response = await _fixture.AdminClient
    ...
    {
        // Arrange & Act - 获取种子admin用户
        var response = await _fixture.AdminClient
            .GetAsync($"/api/v1/users/{WebApiFixture.AdminUserId}");
        ...
        body!.Success.Should().BeTrue();
        body.Data.Should().NotBeNull();
        body.Data!.Id.Should().Be(WebApiFixture.AdminUserId);
        body.Data.UserName.Should().Be("admin");
        body.Data.RealName.Should().Be("系统管理员");
    ...
    public async Task ToggleStatus_ShouldChangeUserStatus()
    {
        ...
        // Act - 切换状态
        var toggleResponse = await _fixture.AdminClient
    ...
    public async Task ChangePassword_WithValidOldPassword_ShouldSucceed()
    {
        ...
        var changeResponse = await _fixture.AdminClient
    ...
    public async Task ChangePassword_WithWrongOldPassword_ShouldFail()
    {
        ...
        var response = await _fixture.AdminClient
            .PutAsJsonAsync(
                $"/api/v1/users/{WebApiFixture.DoctorUserId}/change-password",
                changeRequest);
    ...
    public async Task ChangeProfile_ShouldUpdateRealNameAndPhone()
    {
        ...
        var profileResponse = await _fixture.AdminClient
