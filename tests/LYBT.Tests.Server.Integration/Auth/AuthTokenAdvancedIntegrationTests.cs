...
public class AuthTokenAdvancedIntegrationTests
{
    ...
    public AuthTokenAdvancedIntegrationTests(WebApiFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    public async Task RefreshToken_AfterRevocation_ShouldReturn401()
    {
        ...
        var loginResponse = await _fixture.AnonymousClient.PostAsJsonAsync("/api/v1/auth/login", loginRequest);
        loginResponse.StatusCode.Should().Be(HttpStatusCode.OK);

        var loginResult = await loginResponse.Content.ReadFromJsonAsync<LYBT.Shared.Models.Contracts.Common.ApiResponse<LoginResponse>>(JsonOptions);
        ...
        var refreshResponse = await _fixture.AnonymousClient.PostAsJsonAsync("/api/v1/auth/refresh", refreshRequest);

        // Assert
        _output.WriteLine($"刷新响应状态码: {refreshResponse.StatusCode}");
        refreshResponse.StatusCode.Should().Be(HttpStatusCode.Unauthorized);

        var errorResult = await refreshResponse.Content.ReadFromJsonAsync<LYBT.Shared.Models.Contracts.Common.ApiResponse<LoginResponse>>(JsonOptions);
    ...
    public async Task RefreshToken_Success_ShouldRevokeOldToken()
    {
        ...
        var refreshResult = await refreshResponse.Content.ReadFromJsonAsync<LYBT.Shared.Models.Contracts.Common.ApiResponse<LoginResponse>>(JsonOptions);
