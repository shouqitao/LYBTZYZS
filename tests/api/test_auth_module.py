...
def test_login():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[LOGIN] ✗ 登录失败: {response.text}")
            return False
            
    except Exception as e:
        print(f"[LOGIN] ✗ 登录异常: {str(e)}")
        return False
...
def test_refresh_token():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get("success"):
                print(f"[REFRESH] ✓ 令牌刷新成功")
                return True
            else:
                print(f"[REFRESH] ✗ 令牌刷新失败: {data.get('message')}")
                return False
        else:
            print(f"[REFRESH] ✗ 令牌刷新失败，状态码: {response.status_code}")
            return False
            
    except Exception as e:
        print(f"[REFRESH] ✗ 令牌刷新异常: {str(e)}")
        return False
...
def test_change_password():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get("success"):
                print(f"[CHANGE_PWD] ✓ 密码修改成功")
                return True
            else:
                print(f"[CHANGE_PWD] ✗ 密码修改失败: {data.get('message')}")
                return False
        else:
            print(f"[CHANGE_PWD] ✗ 密码修改失败，状态码: {response.status_code}")
            return False
            
    except Exception as e:
        print(f"[CHANGE_PWD] ✗ 密码修改异常: {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                print(f"[LOGOUT] ✗ 登出失败: {data.get('message')}")
                return False
        else:
            print(f"[LOGOUT] ✗ 登出失败，状态码: {response.status_code}")
            return False
            
    except Exception as e:
        print(f"[LOGOUT] ✗ 登出异常: {str(e)}")
        return False
