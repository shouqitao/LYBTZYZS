...
namespace LYBT.Desktop.Core.Models.Consultation
{
    /// <summary>
    /// 看诊创建信息模型
    /// UltraThink四层架构：Info层，用于UI看诊创建操作的数据收集和验证
    /// </summary>
    public class ConsultationCreateInfo : BindableBase
    {
        #region 基础信息
        ...
        public bool CanSubmit => !IsSubmitting && PatientId != Guid.Empty && DoctorId != Guid.Empty && !string.IsNullOrWhiteSpace(Diagnosis);
        ...
        public ConsultationCreateInfo()
        {
            // 属性变更监听
            PropertyChanged += (sender, e) =>
            {
                if (e.PropertyName == nameof(PatientId) || e.PropertyName == nameof(DoctorId) || 
                    e.PropertyName == nameof(Diagnosis) || e.PropertyName == nameof(IsSubmitting))
                {
        ...
        public (bool IsValid, string? ErrorMessage) Validate()
        {
            ...
            // 验证字段长度
            if (!string.IsNullOrEmpty(ChiefComplaint) && ChiefComplaint.Length > 500)
                return (false, "主诉长度不能超过500个字符");

            if (!string.IsNullOrEmpty(PresentIllness) && PresentIllness.Length > 1000)
                return (false, "现病史长度不能超过1000个字符");

            if (!string.IsNullOrEmpty(PastHistory) && PastHistory.Length > 1000)
                return (false, "既往史长度不能超过1000个字符");
        ...
        /// 设置患者信息
        /// </summary>
        public void SetPatientInfo(Guid patientId, string patientName, int? age = null, string? gender = null)
        {
            PatientId = patientId;
        ...
        /// 清空中医四诊信息
        /// </summary>
        public void ClearTCMFourDiagnosis()
        {
            Inspection = null;
            AuscultationOlfaction = null;
            Inquiry = null;
            Palpation = null;
            TongueInspection = null;
            PulseCondition = null;
        }
