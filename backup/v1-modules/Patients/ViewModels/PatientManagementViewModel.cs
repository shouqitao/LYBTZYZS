...
namespace LYBT.Desktop.Patients.ViewModels
{
    ...
    public class PatientManagementViewModel : NewBaseListViewModel<PatientDto>
    {
        ...
        public DelegateCommand<PatientViewModel> EditCommand { get; }
        public DelegateCommand<PatientViewModel> DeleteCommand { get; }
        public DelegateCommand<PatientViewModel> ToggleStatusCommand { get; }
        public DelegateCommand<PatientViewModel> ViewDetailsCommand { get; }
        ...
        private void UpdatePatientViewModels(System.Collections.Generic.List<PatientDto> patientDtos)
        {
            ...
            {
                // 转换为PatientInfoClean
                var patientInfo = _mapper.Map<PatientInfoClean>(dto);
                
                // 创建PatientViewModel
                var patientViewModel = PatientViewModel.Create(patientInfo);
        ...
        private async Task EditPatientAsync(PatientViewModel patientViewModel)
        {
            ...
            {
                Logger.LogError(ex, "编辑患者失败: {PatientId}", patientViewModel.Id);
        ...
        private async Task ToggleStatusAsync(PatientViewModel patientViewModel)
        {
            ...
            if (confirm)
            {
                ...
                {
                    Logger.LogError(ex, "切换患者状态失败: {PatientId}", patientViewModel.Id);
...
" +
                                   $"职业: {patient.Occupation ?? "未填写"}
" +
                                   $"紧急联系人: {patient.EmergencyContact ?? "未填写"}
" +
                                   $"紧急联系电话: {patient.EmergencyPhone ?? "未填写"}
" +
                                   $"创建时间: {patient.CreateTime:yyyy-MM-dd HH:mm}";
            ...
            {
                Logger.LogError(ex, "查看患者详情失败: {PatientId}", patientViewModel.Id);
        ...
        private async Task BatchEnableAsync()
        {
            ...
            if (confirm)
            {
                try
                {
                    var ids = selectedPatients.Select(p => p.Id).ToList();
                    var result = await _patientService.BatchEnableAsync(ids);

                    if (result.IsSuccess)
                    {
                        await RefreshDataAsync();
                        await _dialogService.ShowInformationAsync($"已成功启用 {result.Data} 个患者", "成功");
                    }
                    else
                    {
                        await _dialogService.ShowErrorAsync(
        ...
        private async Task BatchDisableAsync()
        {
            ...
            if (confirm)
            {
                try
                {
                    ...
                    {
                        await RefreshDataAsync();
                        await _dialogService.ShowInformationAsync($"已成功禁用 {result.Data} 个患者", "成功");
                    }
                    else
                    {
                        await _dialogService.ShowErrorAsync(
