...
// UltraThink四层架构重构：使用新的三层架构组件实现药材管理
...
namespace LYBT.Desktop.Herbs.ViewModels
{
    ...
    public class HerbManagementViewModel : NewBaseListViewModel<HerbDto>
    {
        ...
        public DelegateCommand<HerbViewModel> EditCommand { get; }
        public DelegateCommand<HerbViewModel> DeleteCommand { get; }
        public DelegateCommand<HerbViewModel> ToggleStatusCommand { get; }
        public DelegateCommand<HerbViewModel> ViewDetailsCommand { get; }
        ...
        private void UpdateHerbViewModels(System.Collections.Generic.List<HerbDto> herbDtos)
        {
            ...
            {
                // 转换为HerbInfoClean
                var herbInfo = _mapper.Map<HerbInfoClean>(dto);
                
                // 创建HerbViewModel
                var herbViewModel = HerbViewModel.Create(herbInfo);
        ...
        private async Task EditHerbAsync(HerbViewModel herbViewModel)
        {
            ...
            {
                Logger.LogError(ex, "编辑药材失败: {HerbId}", herbViewModel.Id);
        ...
        private async Task ToggleStatusAsync(HerbViewModel herbViewModel)
        {
            ...
            if (confirm)
            {
                ...
                {
                    Logger.LogError(ex, "切换药材状态失败: {HerbId}", herbViewModel.Id);
        ...
        private async Task ViewDetailsAsync(HerbViewModel herbViewModel)
        {
            ...
            {
                Logger.LogError(ex, "查看药材详情失败: {HerbId}", herbViewModel.Id);
        ...
        private async Task BatchEnableAsync()
        {
            ...
            if (confirm)
            {
                try
                {
                    ...
                    var result = await _herbService.BatchEnableAsync(ids);
