...
namespace LYBT.Desktop.Prescriptions.ViewModels
{
    ...
    public class PrescriptionManagementViewModel : NewBaseListViewModel<PrescriptionDto>
    {
        ...
        public DelegateCommand<PrescriptionViewModel> EditCommand { get; }
        public DelegateCommand<PrescriptionViewModel> DeleteCommand { get; }
        public DelegateCommand<PrescriptionViewModel> CopyCommand { get; }
        public DelegateCommand<PrescriptionViewModel> ViewDetailsCommand { get; }
        public DelegateCommand<PrescriptionViewModel> PrintCommand { get; }
        public DelegateCommand<PrescriptionViewModel> CompleteCommand { get; }
        public DelegateCommand<PrescriptionViewModel> VoidCommand { get; }
        public DelegateCommand BatchCompleteCommand { get; }
        public DelegateCommand BatchVoidCommand { get; }
        ...
        protected override async Task<ServiceResult<PagedResult<PrescriptionDto>>> LoadDataAsync(PagedQueryBaseDto request)
        {
            ...
            var prescriptionQuery = new PrescriptionPagedQueryDto
            {
                ...
                Status = GetStatusFromFilter(SelectedStatusFilter),
                StartDate = StartDate,
                EndDate = EndDate
            };
        ...
        private void UpdatePrescriptionViewModels(System.Collections.Generic.List<PrescriptionDto> prescriptionDtos)
        {
            ...
            {
                // 转换为PrescriptionInfoClean
                var prescriptionInfo = _mapper.Map<PrescriptionInfoClean>(dto);
                
                // 创建PrescriptionViewModel
                var prescriptionViewModel = PrescriptionViewModel.Create(prescriptionInfo);
        ...
        private async Task EditPrescriptionAsync(PrescriptionViewModel prescriptionViewModel)
        {
            ...
            {
                // TODO: 实现处方编辑对话框
                await _dialogService.ShowInformationAsync($"编辑处方 {prescriptionViewModel.PrescriptionNumber} 功能开发中", "提示");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "编辑处方失败: {PrescriptionId}", prescriptionViewModel.Id);
        ...
        private async Task DeletePrescriptionAsync(PrescriptionViewModel prescriptionViewModel)
        {
            ...
            var confirm = await _dialogService.ShowConfirmationAsync(
                $"确定要删除处方 {prescriptionViewModel.PrescriptionNumber} 吗？\n患者：{prescriptionViewModel.PatientName}\n此操作不可恢复。",
                "确认删除");
            ...
            if (confirm)
            {
                ...
                {
                    Logger.LogError(ex, "删除处方失败: {PrescriptionId}", prescriptionViewModel.Id);
                    await _dialogService.ShowErrorAsync($"处方删除失败: {ex.Message}", "错误");
                }
                finally
                {
        ...
        private async Task CopyPrescriptionAsync(PrescriptionViewModel prescriptionViewModel)
        {
            ...
            try
            {
                ...
                var result = await _prescriptionService.CopyAsync(prescriptionViewModel.Id);

                if (result.IsSuccess && result.Data != null)
                {
                    await RefreshDataAsync();
                    await _dialogService.ShowInformationAsync($"处方复制成功\n新处方编号：{result.Data.PrescriptionNumber}", "成功");
                }
                else
                {
                    await _dialogService.ShowErrorAsync(
            ...
            {
                Logger.LogError(ex, "复制处方失败: {PrescriptionId}", prescriptionViewModel.Id);
                await _dialogService.ShowErrorAsync($"复制处方失败: {ex.Message}", "错误");
            }
            finally
            {
                prescriptionViewModel.IsLoading = false;
        ...
        private async Task ViewDetailsAsync(PrescriptionViewModel prescriptionViewModel)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess && result.Data != null)
                {
                    ...
                    await _dialogService.ShowInformationAsync(detailInfo, $"处方详情 - {result.Data.PrescriptionNumber}");
                }
                else
                {
                    await _dialogService.ShowErrorAsync(
            ...
            {
                Logger.LogError(ex, "查看处方详情失败: {PrescriptionId}", prescriptionViewModel.Id);
                await _dialogService.ShowErrorAsync($"查看处方详情失败: {ex.Message}", "错误");
            }
            finally
            {
                prescriptionViewModel.IsLoading = false;
        ...
        private async Task PrintPrescriptionAsync(PrescriptionViewModel prescriptionViewModel)
        {
            ...
            {
                Logger.LogError(ex, "打印处方失败: {PrescriptionId}", prescriptionViewModel.Id);
                await _dialogService.ShowErrorAsync($"打印处方失败: {ex.Message}", "错误");
            }
            finally
            {
        ...
        private async Task CompletePrescriptionAsync(PrescriptionViewModel prescriptionViewModel)
        {
            ...
            var confirm = await _dialogService.ShowConfirmationAsync(
                $"确定要完成处方 {prescriptionViewModel.PrescriptionNumber} 吗？",
                "确认完成");
            ...
            if (confirm)
            {
                try
                {
                    ...
                    var result = await _prescriptionService.CompleteAsync(prescriptionViewModel.Id);
                ...
                {
                    Logger.LogError(ex, "完成处方失败: {PrescriptionId}", prescriptionViewModel.Id);
                    await _dialogService.ShowErrorAsync($"完成处方失败: {ex.Message}", "错误");
                }
                finally
                {
                    prescriptionViewModel.IsLoading = false;
        ...
        private async Task VoidPrescriptionAsync(PrescriptionViewModel prescriptionViewModel)
        {
            ...
            var confirm = await _dialogService.ShowConfirmationAsync(
                $"确定要作废处方 {prescriptionViewModel.PrescriptionNumber} 吗？\n原因：{reason}",
                "确认作废");
            ...
            if (confirm)
            {
                try
                {
                    ...
                    var result = await _prescriptionService.VoidAsync(prescriptionViewModel.Id, reason);
                ...
                {
                    Logger.LogError(ex, "作废处方失败: {PrescriptionId}", prescriptionViewModel.Id);
                    await _dialogService.ShowErrorAsync($"作废处方失败: {ex.Message}", "错误");
                }
                finally
                {
        ...
        private async Task BatchCompleteAsync()
        {
            ...
            if (confirm)
            {
                try
                {
                    var ids = selectedPrescriptions.Select(p => p.Id).ToList();
                    var result = await _prescriptionService.BatchUpdateStatusAsync(ids, PrescriptionStatus.Completed);
        ...
        private async Task BatchVoidAsync()
        {
            ...
            if (confirm)
            {
                ...
                {
                    var ids = selectedPrescriptions.Select(p => p.Id).ToList();
                    var result = await _prescriptionService.BatchUpdateStatusAsync(ids, PrescriptionStatus.Voided, reason);
                ...
                {
                    Logger.LogError(ex, "批量作废处方失败");
                    await _dialogService.ShowErrorAsync($"批量作废失败: {ex.Message}", "错误");
                }
            }
        }
        ...
        private void ClearFilters()
        {
            ...
            StartDate = null;
            EndDate = null;
            SearchManager?.ClearSearchTerm();
        }

        private PrescriptionStatus? GetStatusFromFilter(string statusFilter)
        {
            return statusFilter switch
            {
                "草稿" => PrescriptionStatus.Draft,
                "已完成" => PrescriptionStatus.Completed,
                "已支付" => PrescriptionStatus.Paid,
                "已发药" => PrescriptionStatus.Dispensed,
                "已作废" => PrescriptionStatus.Voided,
                _ => null
            };
        }
        ...
        private async Task ExportPrescriptionsAsync()
        {
            ...
            {
                Logger.LogError(ex, "导出处方失败");
                await _dialogService.ShowErrorAsync($"导出处方失败: {ex.Message}", "错误");
            }
        }
