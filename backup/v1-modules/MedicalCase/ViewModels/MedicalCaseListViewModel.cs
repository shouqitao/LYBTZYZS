...
namespace LYBT.Desktop.MedicalCase.ViewModels
{
    ...
    public class MedicalCaseListViewModel : NewBaseListViewModel<MedicalCaseDto>
    {
        ...
        public DelegateCommand<MedicalCaseViewModel> EditCommand { get; }
        public DelegateCommand<MedicalCaseViewModel> DeleteCommand { get; }
        public DelegateCommand<MedicalCaseViewModel> ViewDetailsCommand { get; }
        public DelegateCommand<MedicalCaseViewModel> StartConsultationCommand { get; }
        public DelegateCommand<MedicalCaseViewModel> CompleteCommand { get; }
        public DelegateCommand<MedicalCaseViewModel> CancelCommand { get; }
        public DelegateCommand BatchStartConsultationCommand { get; }
        ...
        public DelegateCommand ShowTodayCasesCommand { get; }
        public DelegateCommand ShowUrgentCasesCommand { get; }
        ...
        private void UpdateMedicalCaseViewModels(System.Collections.Generic.List<MedicalCaseDto> medicalCaseDtos)
        {
            ...
            {
                // 转换为MedicalCaseInfoClean
                var medicalCaseInfo = _mapper.Map<MedicalCaseInfoClean>(dto);
                
                // 创建MedicalCaseViewModel
                var medicalCaseViewModel = MedicalCaseViewModel.Create(medicalCaseInfo);
        ...
        private async Task BatchStartConsultationAsync()
        {
            ...
            if (confirm)
            {
                try
                {
                    var ids = selectedMedicalCases.Select(mc => mc.Id).ToList();
                    var result = await _medicalCaseService.BatchUpdateStatusAsync(ids, MedicalCaseStatus.InConsultation);

                    if (result.IsSuccess)
                    {
                        await RefreshDataAsync();
                        await _dialogService.ShowInformationAsync($"已成功开始看诊 {result.Data} 个医疗案例", "成功");
                    }
                    else
                    {
                        await _dialogService.ShowErrorAsync(
                            result.ErrorMessage ?? "批量开始看诊失败",
                            "错误");
                    }
                }
                catch (Exception ex)
        ...
        private async Task BatchCompleteAsync()
        {
            ...
            if (confirm)
            {
                ...
                {
                    var ids = selectedMedicalCases.Select(mc => mc.Id).ToList();
                    var result = await _medicalCaseService.BatchUpdateStatusAsync(ids, MedicalCaseStatus.Completed);

                    if (result.IsSuccess)
                    {
                        await RefreshDataAsync();
                        await _dialogService.ShowInformationAsync($"已成功完成 {result.Data} 个医疗案例", "成功");
                    }
                    else
                    {
                        await _dialogService.ShowErrorAsync(
        ...
        private async Task BatchCancelAsync()
        {
            ...
            if (confirm)
            {
                ...
                {
                    var ids = selectedMedicalCases.Select(mc => mc.Id).ToList();
                    var result = await _medicalCaseService.BatchUpdateStatusAsync(ids, MedicalCaseStatus.Cancelled, reason);

                    if (result.IsSuccess)
                    {
                        await RefreshDataAsync();
                        await _dialogService.ShowInformationAsync($"已成功取消 {result.Data} 个医疗案例", "成功");
                    }
                    else
                    {
                        await _dialogService.ShowErrorAsync(
                            result.ErrorMessage ?? "批量取消失败",
                            "错误");
                    }
                }
                catch (Exception ex)
        ...
        private MedicalCaseStatus? GetStatusFromFilter(string statusFilter)
        {
            return statusFilter switch
            {
                "已挂号" => MedicalCaseStatus.Registered,
                "看诊中" => MedicalCaseStatus.InConsultation,
                "已完成" => MedicalCaseStatus.Completed,
                "已取消" => MedicalCaseStatus.Cancelled,
                _ => null
            };
        }
