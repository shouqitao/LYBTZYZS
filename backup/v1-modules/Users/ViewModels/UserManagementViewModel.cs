...
namespace LYBT.Desktop.Users.ViewModels
{
    ...
    public class UserManagementViewModel : NewBaseListViewModel<UserDto>
    {
        ...
        public DelegateCommand<UserViewModel> EditCommand { get; }
        public DelegateCommand<UserViewModel> DeleteCommand { get; }
        public DelegateCommand<UserViewModel> ResetPasswordCommand { get; }
        public DelegateCommand<UserViewModel> ToggleStatusCommand { get; }
        ...
        private void UpdateUserViewModels(System.Collections.Generic.List<UserDto> userDtos)
        {
            ...
            {
                // 转换为UserInfoClean
                var userInfo = _mapper.Map<UserInfoClean>(dto);
                
                // 创建UserViewModel
                var userViewModel = UserViewModel.Create(userInfo);
        ...
        private async Task EditUserAsync(UserViewModel userViewModel)
        {
            ...
            {
                Logger.LogError(ex, "编辑用户失败: {UserId}", userViewModel.Id);
        ...
        private async Task ResetPasswordAsync(UserViewModel userViewModel)
        {
            ...
            if (confirm)
            {
                ...
                {
                    Logger.LogError(ex, "重置密码失败: {UserId}", userViewModel.Id);
                    await _dialogService.ShowErrorAsync($"重置密码失败: {ex.Message}", "错误");
                }
                finally
                {
        ...
        private async Task ToggleStatusAsync(UserViewModel userViewModel)
        {
            ...
            if (confirm)
            {
                ...
                {
                    Logger.LogError(ex, "切换用户状态失败: {UserId}", userViewModel.Id);
                    await _dialogService.ShowErrorAsync($"用户{action}失败: {ex.Message}", "错误");
                }
                finally
                {
                    userViewModel.IsLoading = false;
