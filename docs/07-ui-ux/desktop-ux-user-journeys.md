...
- 计数与清单口径：以 `src/Client/Desktop` 源码扫描为准（排除 `bin/`/`obj/`，口径见 [`DESKTOP_ARCHITECTURE_STANDARD.md`](../../src/Client/Desktop/DESKTOP_ARCHITECTURE_STANDARD.md) §13.5）；全量清单见 `desktop-view-inventory.md`。正文一律表述为「代码实际（src/Client/Desktop）」。
