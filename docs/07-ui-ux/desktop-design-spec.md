...
> ⚠️ 本文档超 400 行（约 820 行）。**TL;DR**：设计态 SSOT——原则/主题色彩/布局/导航/交互/业务 UX/控件/对话框/状态反馈/快捷键/无障碍。**边界**：交互细则另见 `desktop-ux-interaction-spec.md`；错误文案见 `desktop-ux-error-handling.md`；加载态见 `desktop-ux-loading-states.md`；页面级规格见 `desktop-ui-detailed-design.md`。**阅读建议**：先 §1 原则 + §3 主题 + §10 状态反馈，再按改的页面跳章节。
