...
created: 2026-06-10
updated: 2026-06-10
sources: ["docs/04-api-reference/09-sync.md"]
---
# 同步冲突处理
...
2.  **状态冲突**:
    *   **ERR-70303 (SyncActiveCaseConflict)**: 本地试图上传一个医案，但服务端该患者已有一个"活跃"状态的医案。这违反了业务规则（通常一个患者同一时间只能有一个活跃医案）。⚠️ MedicalCase 同步当前未实现，此冲突类型为设计预留。
    *   **ERR-70304 (SyncCaseLocked)**: 本地试图更新一个已在服务端被锁定（如已打印或归档）的医案。⚠️ 同上，MedicalCase 同步未实现。
...
*   医案同步因状态机约束需特殊处理：已关闭/已取消的医案不允许覆盖（详见 [医案锁定规则](31-medical-case-locking-rules.md)）。
*   校验和机制：基于 SHA256 of JSON-serialized business fields 的差异检测（详见 [同步模块](modules/sync-module.md) Checksum 流程）。