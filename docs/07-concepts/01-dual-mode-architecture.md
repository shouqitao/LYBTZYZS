...
created: 2026-06-10
updated: 2026-06-10
sources: ["docs/03-architecture/decisions/0002-dual-mode-architecture.md", "docs/03-architecture/01-system-overview.md", "docs/03-architecture/05-dual-mode.md"]
---
...
- **敏感数据处理**: 尽管本地模式在局域网或单机运行，仍需遵循[敏感数据分级与保护](10-sensitive-data.md)原则。对身份证号、手机号等字段需进行脱敏或加密存储（v1.0 初期主要依赖 OS 权限控制，后续版本将逐步强化应用层加密）。
