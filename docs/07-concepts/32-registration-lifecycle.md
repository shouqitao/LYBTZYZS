...
*   **关键特征**：
    ...
    *   **原医案恢复**：若挂号回退至 `Waiting`，医生再次接诊时，系统应**恢复**原医案（撤销软删除），而非创建新医案，以保留之前的诊断草稿。> ⚠️ **当前未实现**：MedicalCase 模块尚无 `RestoreAsync` 能力（其他实体如 Patient/Herb/Formula 均已实现）。回退场景下目前会创建新医案而非恢复原医案。
...
*   [用户角色权限](../01-product/04-user-roles.md)
