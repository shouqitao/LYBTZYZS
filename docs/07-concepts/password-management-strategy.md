...
related: [authentication, superadmin-architecture, configuration-management, testing-strategy]
created: 2026-06-10
updated: 2026-06-10
sources: ["docs/05-development/security-password-management.md"]
---
# 密码管理与安全策略

## 概述

密码管理与安全策略定义了凌隐宝堂系统中用户凭证的存储、验证、保护及管理员账户的生命周期管理机制。该策略核心在于通过[[authentication|认证授权]]模块底层的 BCrypt 哈希算法、可配置的账户锁定机制以及环境感知的管理员自动化运维流程，确保系统在面对暴力破解、时序攻击及内部误操作时的安全性。

## 核心机制

### 1. BCrypt 密码哈希

系统采用 BCrypt 算法作为密码存储的标准，工作因子（WorkFactor）设定为 **11**。
*   **安全性**：WorkFactor=11 使得单次哈希计算耗时约 200ms，有效增加了暴力破解和彩虹表攻击的计算成本。
*   **自适应性**：通过 `VerifyAndRehashIfNeeded` 方法，系统能够在验证密码的同时检测哈希版本。若未来需要提升 WorkFactor（如升级至 12），系统可在用户登录时自动重新哈希并更新存储，实现平滑迁移。
...
*   **依赖注入**：`BcryptPasswordService` 通过 DI 容器注入。
*   **Mock 支持**：在单元测试中，可以替换为 Mock 实现，模拟各种密码验证场景（如成功、失败、锁定），而无需实际执行耗时的 BCrypt 计算。这符合[[testing-strategy|测试策略]]中关于隔离外部依赖的原则。
...
- [[authentication|认证授权]]
- [[superadmin-architecture|SuperAdmin 架构设计]]
