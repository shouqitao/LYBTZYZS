...
*   **容量评估**: 100MB 的限制需结合业务数据量进行评估。对于 [药材内存缓存](16-herb-cache-strategy.md) 等全量预加载场景，需监控实际占用情况，必要时调整上限。
*   **过期策略**: 合理设置 `DefaultExpirationMinutes` 平衡数据一致性与命中率。高频变动数据应设置较短的过期时间或采用主动失效机制。
*   **监控告警**: 建议集成内存使用率监控，当缓存命中率显著下降或内存占用持续高位时发出告警。

## 关联概念

*   [缓存策略](15-caching-strategy.md): 内存缓存是服务端缓存策略的具体实现之一。
*   [药材内存缓存策略](16-herb-cache-strategy.md): 药材模块重度依赖内存缓存以提升检索性能。