...
created: 2026-06-10
updated: 2026-06-10
sources: ["docs/03-architecture/system-overview.md", "docs/05-development/standards/STD-03-CrossModule.md", "docs/03-architecture/shared-layer.md"]
---
# 跨模块通信机制
...
* **错误传播**：跨模块调用产生的异常应遵循 `[[exception-hierarchy|异常类型体系]]`，并在调用方进行适当的捕获和处理，避免底层实现细节泄露。
...
3. **单向数据流**：通信应遵循清晰的数据流向，严格避免循环调用，是 `[[unidirectional-dependency|单向依赖原则]]` 在运行时交互层面的具体落地。
4. **契约共享**：跨模块接口通常定义在 `[[shared-layer-architecture|共享层架构]]` 中，确保 Server 和 Client 端均可引用统一契约。

## 相关概念
* [[unidirectional-dependency|单向依赖原则]]
* [[component-decomposition|组件分解]]
* [[server-architecture|服务端架构]]
* [[shared-layer-architecture|共享层架构]]
* [[exception-hierarchy|异常类型体系]]
