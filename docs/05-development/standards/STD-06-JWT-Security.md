...
| Token 类型 | 有效期 | 用途 |
|------------|--------|------|
| Access Token | 30 分钟（可配置，5-1440 分钟） | API 请求认证，短生命周期降低泄露风险 |
| Refresh Token | 7 天滑动 + 30 天绝对过期 | 静默续签 Access Token，用户活跃时自动刷新 |
...
| 日期 | 变更 |
|------|------|
| 2026-06-25 | 修正 Token 时效: Access Token 15min → 30min（可配置）; Refresh Token 7天 → 7天滑动+30天绝对过期 |
