...
| **多角色协作断层** | 前台登记、医生诊疗、管理员审核信息不互通 | 候诊进度不可见，审核靠人工翻阅 |
...
| GOAL-05 | **支持离线诊疗** | 本地模式支持无网络环境下的完整诊疗，事后与服务端同步 |
...
**流程**：切换本地模式 → 完整接诊 → 网络恢复 → 数据同步

**关键价值点**：
- 本地 LocalDB 支持断网环境下完整诊疗流程（含患者查询、医案创建、处方开具）
- 事后手动同步，Checksum 差异检测仅同步变更数据
...
| 数据同步 (Sync) | 8 | 5 阶段工作流、Checksum 冲突检测、逐项解决 |
| 平台基础设施 (Platform) | 35 | Shell/Config/Error/Logging/Health/CardReader 合并 |
