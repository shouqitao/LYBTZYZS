...
| **代码实现与 PRD 一致性** | **深度扫描发现 257 项偏差 (73 P1 / 121 P2 / 63 P3)，详见 5.4** | **65/100** |
| **综合评分** | | **80/100** |

### 5.4 代码实现偏差分析 (PRD vs Code 深度扫描)

> **扫描日期**: 2026-02-21
> **完整报告**: [prd-code-deep-scan-report.md](2026-02-21-prd-code-deep-scan-report.md)

#### 5.4.1 扫描概述

对 14 个功能模块的 131+ FR 进行四维度深度扫描 (存在性/完整度/准确性/双模式覆盖)。
...
| 质量评级 | 模块 | 偏差密度 | P1 | P2 | P3 |
|----------|------|----------|-----|-----|-----|
| A (优秀) | card-reader | 0.50 | 0 | 0 | 1 |
| A (优秀) | health-diagnostics | 0.56 | 0 | 2 | 3 |
| A (优秀) | nfr | 0.45 | 2 | 4 | 3 |
| B (良好) | error-handling | 1.13 | 3 | 3 | 3 |
| B (良好) | logging | 1.14 | 1 | 3 | 4 |
| B (良好) | formulas | 1.38 | 2 | 10 | 6 |
| C (一般) | auth | 1.62 | 2 | 11 | 8 |
| C (一般) | herbs | 1.85 | 3 | 16 | 5 |
| C (一般) | desktop-shell | 2.00 | 1 | 9 | 4 |
| C (一般) | configuration | 2.00 | 0 | 5 | 3 |
| C (一般) | medical-cases | 2.06 | 7 | 22 | 8 |
| D (较差) | patients | 2.15 | 18 | 6 | 4 |
| D (较差) | sync | 2.25 | 5 | 8 | 5 |
| D (较差) | users | 2.50 | 17 | 12 | 1 |
| F (极差) | printing | 6.75 | 12 | 10 | 5 |
...
| # | 问题 | 影响模块 | 严重度 | 建议修复方式 | 预估消除偏差数 |
|---|------|---------|--------|-------------|---------------|
| X1 | 错误码体系脱节 (MCCEE) | 几乎全部 | P1 | 统一 MCCEE 编码 | ~40 |
| X2 | 本地模式功能缺口 | users, herbs, formulas, patients, sync | P2 | 逐模块补齐 IDataSource | ~20 |
| X3 | Token Family 撤销未联动 | users, auth | P1 | 5 场景统一补齐 | ~8 |
| X4 | Service 层未使用 ErrorCode | users, herbs, formulas | P2 | Result.Failure 传入 ErrorCode | ~6 |
| X5 | 字段验证值不一致 | herbs, formulas, auth, users | P2 | 统一 ValidationConstants | ~12 |
| X6 | 分页筛选内存过滤 | users, herbs, formulas, medical-cases | P2 | 迁移到 Repository IQueryable | ~4 |
| X7 | 引用检查 CanDelete=true | patients, herbs | P1 | 实现实际引用计数 | ~6 |
| X8 | 打印层级重构 (C6) | medical-cases, printing | P1 | 实体迁移到 MedicalCase 层 | ~15 |
...
#### 横切面专项修复 (深度扫描新增，ROI 最高)

> 以下为 PRD vs Code 深度扫描 (2026-02-21) 发现的跨模块共性问题。
> 作为专项整治而非逐个修复，效率更高。

| # | 专项 | 涉及模块 | 预估消除偏差数 | 预估工时 |
|---|------|---------|---------------|---------|
| X3 | Token Family 撤销联动 (5 场景) | users | ~8 | 1 天 |
| X7 | 引用检查实际查询 (CanDelete) | patients, herbs | ~6 | 0.5 天 |
| X5 | 字段验证值对齐 (ValidationConstants) | herbs, formulas, auth, users, configuration | ~12 | 0.5 天 |
...
| 2026-02-21 | v2.0 | PRD vs Code 深度扫描集成: (1) 新增 5.4 代码实现偏差分析 (257 偏差/8 横切面/质量评级); (2) 更新 5.3 综合评分 88->80; (3) 新增横切面专项修复建议 (8 专项/~111 偏差消除); (4) 链接深度扫描报告 |
