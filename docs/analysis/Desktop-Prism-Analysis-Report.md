...
注意：当前项目基线固定在 .NET 8 + Prism 8.1.97，暂无升级计划。本报告基于历史调研所得，请以 docs/overview/guiding-philosophy.md 为现行标准。

基于Prism官方最佳实践（版本9.0.537，历史调研）对LYBT Desktop项目进行深度分析，发现项目在架构设计上存在多个偏离Prism推荐模式的问题。主要问题集中在模块化设计、依赖注入配置、导航系统使用和事件聚合器实现等方面。
