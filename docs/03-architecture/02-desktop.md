...
SyncPhase 状态机（`Idle → CheckingDifferences → ReviewingDifferences → Syncing → Completed/Failed`）、关键类型（`SyncResultSummary`/`SyncRetryDescriptor`/`SyncErrorCategory`）、SyncView 底栏布局、冲突解决 UI（左右对比 + 差异高亮 + 保留本地/使用服务端/跳过）的完整设计已并入 [17-sync-protocol.md](17-sync-protocol.md) 的「同步状态机」「错误分类」段。
