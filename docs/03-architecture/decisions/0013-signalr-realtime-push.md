...
> 注：「实时推送」诉求**仅存在于远程模式**（有前台挂号→有待诊队列→需推送）。本地模式默认医生独立（无前台用户时无队列）；即使建前台用户有队列，本地也不部署 SignalR（双模式工作流分野见 [08-registration.md](../../02-requirements/08-registration.md)「双模式工作流」段）。
