...
> **范围收敛（2026-06-28，R10 spec S7）**：SignalR 推送**仅远程模式**。本地模式**不做推送**——本地模式取消挂号、无待诊队列（医生「来一个看一个」），无队列即无推送需求，本地模式排除在 SignalR 范围之外。
...
> 注：「实时推送」诉求**仅存在于远程模式**（有前台挂号→有待诊队列→需推送）。本地模式无前台、无挂号、无队列（见 [R10 spec S3](../../compose/specs/2026-06-28-registration-workflow-redesign.md)），推送无适用场景。
...
- 本地模式（[ADR-0009](0009-url-driven-dual-mode.md)）不涉及推送通道——Registration 模块在本地不激活，无队列无推送
...
| 日期 | 变更 | 原因 |
|------|------|------|
| 2026-06-28 | 范围收敛：本地模式排除（无队列无推送），仅远程；关闭「双模式推送机制」待决项 | R10 spec S7：本地取消挂号、无队列，推送无适用场景 |
