...
| 2026-08-06 | **架构测试项目重构（commit `3df76c2c1`）**：① 创建 `TestAssemblies.cs` 公共程序集清单（Server/Desktop/All），消除各测试文件重复定义；② 重构 4 个测试文件引用 `TestAssemblies`；③ 删除 9 个过时测试（UltraThink/Record-Only 残留：Pipeline/Workflow/Bus/Engine 命名检查、Intelligence/StateMachine 检查、TransactionPattern 检查、Batch2_ServiceRegistration 检查）；④ 创建 `ARCHITECTURE-RULES.md` 规则映射表（30 条规则→测试方法→文档引用）。验证：build --no-incremental 0 错误 0 警告、架构测试 83/83 全部通过 | 程序集清单统一（Reports/Registration 覆盖）；无重复测试；删除所有 UltraThink/Record-Only 残留 | 技术总监 |
