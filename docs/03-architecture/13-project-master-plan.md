...
| A-27 | 技术栈减法（方案 A） | 删除死重量依赖：BCrypt（PasswordHelper 哈希/验证零调用，Identity PBKDF2 取代）/ Swagger（评估保留：完整接线）/ Velopack（0 引用，未引入）/ Sqlite（LocalDB 取代）；蓝图新增「技术栈合理性评估」章节。任务书：`docs/compose/specs/task-a27-stack-subtraction-2026-08-08.md` | A-26 | ✅ `846cb5411`（报告 `docs/compose/reports/a27-stack-subtraction.md`） | 0.5d |
