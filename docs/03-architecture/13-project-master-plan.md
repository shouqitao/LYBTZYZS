...
| A-31 | C 批次执行（整合方案落地） | C-0 P0 缺陷修复（LocalWebAPI 策略/MedicalCase 验证/Reports 双轨策略）/ C-1 日志集中（Shared.Logging 升级 AddLybtLogging M1-M10）/ C-2 异常统一（处理器收敛+死类删除）/ C-3 Server 合并（Herbs+Formula→Catalog + Auth+Users→Identity）/ C-4 Desktop 合并 / C-5 机制收敛 / C-6 死代码清理 | A-30 S4 定稿 `fa18f67b3` | 🟡 C-0 已派发 | 6-10d |
