...
| E-04 规则体系：MCP 配置禁用问题工具（tavily/serena） | ✅ | 2026-08-04 | 本地修改（`.mimocode` 被 gitignore，无 commit） |
...
| 2026-08-04 | 规则体系三层架构优化：角色层=通用工作流/调度/沟通规则，项目层=入口+引用，Skill=项目规则 SSOT（v0.6.0）；沟通规则归位角色层；项目 AGENTS.md 141→58 行；MCP 禁用 tavily/serena（.mimocode 被 gitignore，本地生效）；优化报告已完成使命后删除 | 消除规则分散重复（重复率约 40%），单点维护 | 技术总监 |
