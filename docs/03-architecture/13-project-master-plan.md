...
| 2026-08-08 | **决策点 2/4 定案（A-31-C8，用户按技术总监推荐确认）**：① 决策点 2 跨模块门面 = **删统一门面 `ICrossModuleService`，只留 6 个域接口**（`IXxxCrossModuleService`，消费方按方法归属改注入域接口）——符合模块自治哲学，消除门面双轨；② 决策点 4 映射双机制 = **直用 DTO + Mapperly 唯一**（workspace 保存链路改走 Mapperly，删 `DtoConversionExtensions` 手写扩展）——A-26 P2-11 最终落地。蓝图 §2.1 接口矩阵 + §3.5 映射规则已同步（先文档后代码）。任务书：`docs/compose/specs/task-a31-c8-decision-points-convergence-2026-08-08.md` | 多方案并存收敛（与密码/日志/异常同性质）；两决策点为 A-30 整合方案遗留 | 技术总监 |
