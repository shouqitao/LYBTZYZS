...
| ID | 任务 | 内容 | 依赖 | 状态 | 预估 |
|----|------|------|------|------|------|
| G-01 | 矛盾统一（27 条） | 按 §九 权限终局裁决统一 US-MC-001/药材/验方/D7/医案取消/SQLite/Prism/Token 等文档表述；6 条核实代码 | 无 | 🟡 | 0.5d |
| G-02 | docs 缺口补写（59 条） | 离线密码重置方案/令牌族撤销表结构/DPAPI 迁移/错误码映射/本地威胁模型等 | G-01 | ⬜ | 1d |
| G-03 | SSOT 收敛 | 权限矩阵权威引用统一（04-permissions）、跨文档对齐、wiki 同步 | G-01 | ⬜ | 0.5d |
