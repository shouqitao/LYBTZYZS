...
| 2026-08-11 | R2 需求文档校准（纯文档） | **已执行**：13-traceability-matrix.md 61 处状态校准（v1.1，含 T4 修复后状态）+ 统计汇总表重算（141 US：✅94/⚠️30/🔴10/🧲7）；12 个需求文档 123 处 US 状态段同步；WebAPI README 3 个 stale 端点（batch-details/consultations/prescriptions）删除 | 文档状态列如实反映代码现状（文档是字典不是过程）；🔴 10 项为剩余代码 backlog | ✅ 待提交 |
