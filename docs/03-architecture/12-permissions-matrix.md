...
| # | 类型 | 问题 | 代码位置 | 修复方向 |
| --- | ------ | ------ | --------- | --------- |
| **K4** | 💻 | **已修复**：`IdentitySeedData.ResolveSysAdminPassword` 生产必须环境变量 `DefaultPasswords__SysAdminPassword`（缺失抛异常禁回退），不再有明文默认密码 | `IdentitySeedData.cs` | 已实施（`IdentitySeedData.ResolveSysAdminPassword` + `DatabaseInitializationService.ValidateSetupToken`） |
...
| # | 类型 | 问题 | 代码位置 | 修复方向 |
| --- | ------ | ------ | --------- | --------- |
| **K7** | ✅ | **已修复**：`StartVisit` 原子创建 MedicalCase(Active)+Registration(InProgress)+返回 MedicalCaseId，接诊链断裂已修复 | `RegistrationsController.cs` | 已完成 |
...
| **K3** | ✅ | **已修复**：`PolicyConstants.DoctorOnly` 已存在，`MedicalCasesController` 创建端点已使用。恢复/纯 Admin 场景需新增纯 Admin 策略（见 04-permissions §2.2 注） | `PolicyConstants.cs` | 已完成 |
