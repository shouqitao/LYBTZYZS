...
### 1.1 模块定位
Patients模块是WPF桌面客户端的核心业务模块之一，负责患者信息的管理功能。该模块采用简化设计理念，专为小型中医诊所的实际需求而设计，避免过度工程化。

### 1.2 核心功能状态

#### 1.2.1 设计完成📋
- **患者管理设计**：创建、编辑、查询患者等功能规划
- **患者列表设计**：分页查询、搜索、筛选功能设计
- **患者详情设计**：完整患者信息展示、就诊历史设计
- **患者档案设计**：健康档案管理、病史记录设计

#### 1.2.2 基础实现✅
- **PatientsModule**: Prism模块注册框架
- **基础服务注册**: IPatientService接口和实现类框架

#### 1.2.3 待实现⚠️
- **患者列表管理**：分页查询、搜索、筛选功能待实现
- **患者详情查看**：完整信息展示、就诊历史显示待实现
- **患者编辑功能**：基本信息、健康档案编辑待实现
- **患者档案管理**：病史记录、健康档案管理待实现
- **就诊历史查询**：历史记录查询和统计待实现
...
### 4.1 MVVM架构映射（设计目标）
- **Model**: PatientDto（共享层）+ PatientItem（UI模型）
- **View**: PatientsMainView + PatientsListView + PatientEditView
- **ViewModel**: PatientsMainViewModel + PatientsListViewModel + PatientEditViewModel
...
```csharp
// 待实现的ViewModel基础结构
public class PatientsMainViewModel : ModernViewModelBase
{
    // 属性绑定
...
- **服务层空实现**: PatientService可能只有接口没有实现
...
#### 5.2.1 第一阶段（基础框架）
1. 完成PatientService基础实现
2. 创建PatientsMainViewModel框架
3. 创建PatientsMainView基础界面
...
- LYBT.Shared.Interfaces.Services.IPatientService
- LYBT.Shared.Interfaces.Api.IPatientApi
...
2. **短期目标**: 创建核心ViewModel和View，实现基础患者管理功能
3. **中期目标**: 完善患者功能，添加搜索筛选和详情查看
