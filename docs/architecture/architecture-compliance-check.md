...
> **创建日期**: 2025-10-15
> **适用范围**: LYBT中医诊所系统所有代码
> **关联文档**: [Server模块设计标准](server-module-design-standard.md), [Client端统一设计标准](client/unified-design-standard.md)
> **Project Standardization 3.0**: Task 6.2.2

---

## 1. 概述

本文档定义了LYBT中医诊所系统的架构合规检查标准和自动化检查机制，确保所有代码符合项目的架构设计原则和最佳实践。架构合规检查是代码质量保证的重要组成部分，通过系统化的检查规则和自动化工具，维护代码架构的一致性和可维护性。

### 1.1 架构合规检查目标

- **架构一致性**: 确保所有模块遵循统一的架构模式
- **设计原则合规**: 验证代码符合SOLID、DRY、KISS等设计原则
- **技术标准遵循**: 检查是否遵循项目技术标准和约定
- **依赖关系正确**: 验证模块间依赖关系的合理性
...
// ❌ 禁止的架构模式
Controller → QueryService + BusinessService → Repository  // CQRS
Controller → Repository                                   // 跳过Service层
Controller → Service → Service → Repository              // 多层Service
```
...
function Test-CQRSCompliance {
    ...
    if ($queryServices.Count -gt 0 -or $businessServices.Count -gt 0) {
        Write-Warning "发现CQRS模式违规: $($queryServices.Count)个QueryService, $($businessServices.Count)个BusinessService"
        return $false
    }
...
- [ ] **单一Service接口**: 每个模块只有一个IXxxService接口
- [ ] **标准三层**: Controller→Service→Repository调用链清晰
- [ ] **无跳层**: Controller不直接调用Repository
- [ ] **无冗余层**: 不存在额外的中间层
...
public class PatientController : ControllerBase
{
    ...
    {
        var patients = await _repository.GetAllAsync(); // ❌ 跳过Service层
        return Ok(patients);
    }
}
```
...
**规则ID**: SRV-SVC-001  
**检查项目**: Service接口设计符合标准规范  
**严重程度**: 🟡 High

**检查标准**:
- Service接口定义在`LYBT.Shared.Interfaces.Services`命名空间
- 接口方法数量在6-12个之间
- 方法命名遵循标准约定
...
public class ServiceInterfaceComplianceAnalyzer
{
    public ComplianceResult AnalyzeServiceInterface(Type serviceInterface)
    {
        ...
        {
            result.AddViolation("Service接口命名空间不正确", serviceInterface.FullName);
        }
        ...
        {
            result.AddViolation($"Service接口方法数量异常: {methodCount}个（标准：6-12个）", serviceInterface.Name);
        }
        ...
        foreach (var method in serviceInterface.GetMethods())
        {
            ...
            {
                result.AddViolation($"Service方法命名不符合Async约定: {method.Name}", serviceInterface.Name);
            }
            
            if (method.Name.Contains(serviceInterface.Name.Replace("I", "")))
            {
                result.AddViolation($"Service方法名不应包含实体名: {method.Name}", serviceInterface.Name);
            }
        }
...
- [ ] **命名空间正确**: Service接口在`LYBT.Shared.Interfaces.Services`
- [ ] **方法数量合规**: 6-12个方法之间
...
### 2.3 Repository架构合规检查

#### 2.3.1 检查规则

**规则ID**: SRV-REPO-001  
**检查项目**: Repository架构符合Specification模式标准  
**严重程度**: 🟡 High
...
public class RepositoryComplianceAnalyzer
{
    public ComplianceResult AnalyzeRepository(Type repositoryType)
    {
        ...
        {
            result.AddViolation("Repository未继承BaseRepository<T>", repositoryType.Name);
        }
        ...
        {
            result.AddViolation("Repository未实现IRepository<T>接口", repositoryType.Name);
        }
        ...
        foreach (var ctor in constructors)
        {
            ...
            {
                result.AddViolation("Repository不应注入Service依赖", repositoryType.Name);
            }
        }
...
- [ ] **基类继承**: Repository实现继承BaseRepository<T>
...
public class ViewModelComplianceAnalyzer
{
    public ComplianceResult AnalyzeViewModel(Type viewModelType)
    {
        ...
        foreach (var ctor in constructors)
        {
            ...
            {
                result.AddViolation("ViewModel不应注入Service依赖（已废弃Service层）", viewModelType.Name);
            }
            ...
            {
                result.AddWarning("ManagementViewModel应注入Repository依赖", viewModelType.Name);
            }
        }
...
public class PatientManagementViewModel : UnifiedListViewModelBase<PatientDto>
{
    ...
    public PatientManagementViewModel(
        IPatientRepository patientRepository,  // 1️⃣ Repository依赖
...
- [ ] **依赖注入**: 优先注入Repository，禁止Service依赖
- [ ] **构造函数顺序**: Repository依赖优先，可选依赖在末尾
- [ ] **异常处理**: 依赖基类的异常处理机制
...
function Test-ModuleStructure {
    ...
    if ($interfaces.Count -eq 0) {
        $result += "Repository接口未在Interfaces目录中"
    }
    
    if ($implementations.Count -eq 0) {
        $result += "Repository实现未在Repositories目录中"
    }
...
- [ ] **目录结构**: 符合标准模块目录结构
- [ ] **接口位置**: Repository接口在Interfaces目录
- [ ] **实现位置**: Repository实现在Repositories目录
- [ ] **无废弃目录**: 不存在Services或Mappings目录
- [ ] **模块注册**: 存在标准的Module.cs文件
- [ ] **文档完整**: 包含README.md文档

### 3.3 Repository实现合规检查

#### 3.3.1 检查规则

**规则ID**: CLI-REPO-001  
**检查项目**: Client端Repository实现符合标准  
**严重程度**: 🟡 High

**检查标准**:
- Repository继承RepositoryBase基类
...
public class ClientRepositoryComplianceAnalyzer
{
    public ComplianceResult AnalyzeClientRepository(Type repositoryType)
    {
        ...
        {
            result.AddViolation("Client Repository未继承RepositoryBase<TDto, TCreateDto, TUpdateDto, TApi>", repositoryType.Name);
        }
        ...
        foreach (var method in repositoryType.GetMethods())
        {
            ...
            {
                result.AddViolation($"Client Repository方法不应返回ServiceResult: {method.Name}", repositoryType.Name);
            }
        }
        ...
        foreach (var ctor in constructors)
        {
            ...
            {
                result.AddViolation("Client Repository应注入IApiClientManager", repositoryType.Name);
            }
        }
...
#### 3.3.2 标准Repository实现

```csharp
// ✅ 标准Client端Repository实现
...
{
    public Guid Id { get; set; }  // ⚠️ Client端UpdateDto需要Id用于API调用
...
public class ArchitectureComplianceChecker : IArchitectureComplianceChecker
{
    ...
    {
        _logger.LogInformation("开始架构合规检查: {ProjectPath}", projectPath);
        ...
        _logger.LogInformation("架构合规检查完成: {OverallLevel} - {TotalViolations}违规, {TotalWarnings}警告",
...
jobs:
  architecture-compliance:
    ...
    - name: Run Architecture Compliance Checker
      run: |
  ...
  - job: ArchitectureCheck
    ...
    - task: PowerShell@2
      ...
      inputs:
        ...
        script: |
          $report = Get-Content "architecture-compliance-report.json" | ConvertFrom-Json
          Write-Host "架构合规检查结果: $($report.overallLevel)"
...
function Invoke-ArchitectureComplianceCheck {
    ...
    Write-Host "🏗️  开始架构合规检查..." -ForegroundColor Blue
    ...
    Write-Host "`n📊 检查结果摘要:" -ForegroundColor Green
    ...
    if ($report.violations.length -gt 0) {
        Write-Host "`n🚨 严重违规项目:" -ForegroundColor Red
    ...
    if ($FailOnCritical -and $report.overallLevel -in @("Critical", "Poor")) {
        Write-Host "`n❌ 架构合规检查失败，请修复违规项目" -ForegroundColor Red
        exit 1
    }
    
    Write-Host "`n✅ 架构合规检查完成" -ForegroundColor Green
}
...
<PackageManifest>
  <Metadata>
    ...
    <DisplayName>LYBT架构合规检查器</DisplayName>
...
<body>
    <h1>🏗️ LYBT架构合规检查报告</h1>
    
    <div class="summary-card">
        <h2>📊 检查摘要</h2>
        <p><strong>总体评级:</strong> <span class="good">Good</span></p>
...
通过持续的架构合规检查和改进，LYBT项目将保持高质量的架构设计，支撑业务的长期发展需求。

---

**文档维护**: 本文档应定期更新，反映最新的架构标准和合规要求。如有疑问或建议，请联系架构团队。
