...
public class ConfigurationValidationService
{
    ...
    private bool ValidateRequiredConfigurations()
    {
        ...
        foreach (var config in requiredConfigs)
        {
            ...
            {
                _logger.LogError("Required configuration missing: {Config}", config);
                return false;
            }
        }
    ...
    private void ValidateSectionSecurity(IConfigurationSection section, string[] sensitivePatterns)
    {
        foreach (var child in section.GetChildren())
        {
            ...
            if (!string.IsNullOrEmpty(value))
            {
                ...
                {
                    _logger.LogWarning("Sensitive configuration detected in section: {SectionPath}", child.Path);
                }
            }
...
public class ConfigurationValidationMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        if (!context.Items.ContainsKey("ConfigurationValidated"))
        {
            ...
            if (!isValid)
            {
                _logger.LogError("Configuration validation failed. Application startup aborted.");
                context.Response.StatusCode = 500;
                await context.Response.WriteAsync("Configuration validation failed");
                return;
            }
...
public class ConfigurationHealthCheck : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return HealthCheckResult.Unhealthy("Database connection string is not configured", data);
            }
            ...
            {
                return HealthCheckResult.Unhealthy("JWT issuer is not configured", data);
            }
...
public class JwtConfigurationValidator
{
    public static bool ValidateJwtConfiguration(IConfiguration configuration)
    {
        ...
        {
            Console.WriteLine("JWT secret key is too short (minimum 32 characters required)");
            return false;
        }

        if (string.IsNullOrEmpty(issuer) || string.IsNullOrEmpty(audience))
        {
            Console.WriteLine("JWT issuer or audience is not configured");
            return false;
        }
...
public class ConfigurationReloadService : IHostedService, IDisposable
{
    ...
    {
        _logger.LogInformation("Configuration has been reloaded");
