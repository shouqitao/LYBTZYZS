...
## Domain 15: 收尾验证（AUTH-000/007 + 覆盖总结）✅

| # | US | 测试项 | 结果 |
|---|-----|--------|:---:|
| 1 | US-AUTH-000 | 首登初始化（sysadmin 专属强制改密） | ✅ 设计正确（mustChangePassword 字段；新用户=False 符合——仅 sysadmin 首登强制） |
| 2 | US-AUTH-007 | 安全审计日志（SecurityAuditService 记录） | ✅ 已实现（AC 仅要求记录——查询走日志文件） |

**User Story 测试最终覆盖**（计划 v2：113 US）：
- ✅ 已测：95（核心域全链 + 认证/用户/患者/药材/验方/挂号/医案/报表/系统维护 + 权限/边界）
- ⏳ 剩余 13：Desktop-only 5（AUTH-009/010/012/013 AutoLogin/本地认证）+ SignalR 2（REG-007/008）+ 谨慎型 2（AUTH-003 限流/006 重放——真机触发会锁账号）+ 已确认正确 4（AUTH-000/007 + MC-008/009 + SYS-004~009 已测）

**今日修复 bug 总清单**（8 个——全部真机发现→修复→真机确认）：
```
7caa27e41 软删唯一索引冲突 | b058a1124 Create 误删 | 18a6f676e Consultation.CreatedBy
3fa497d3e 配置权限隔离 | 4bfc70849 处方明细金额 | 231d15326 电话唯一约束
48278e512 409 状态码偏差 | 63674dc40 用户层级约束
```
