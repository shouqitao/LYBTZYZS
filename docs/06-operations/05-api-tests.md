...
> 📌 **职责说明**：本文档为 Postman 手工 API 测试用例集，侧重端点行为验证与运维验收。与 [05-development/09-postman-vs-dotnet-testing.md](../05-development/09-postman-vs-dotnet-testing.md) 存在职责重叠（后者侧重 Postman vs xUnit 测试策略选型与自动化测试规范）。两者均保留：本文供运维/验收参考，05/09 供开发流程参考。
...
| 模块 | TC 组数 | 测试用例数 | 覆盖场景 |
|------|---------|------------|----------|
| Auth | 5 | 16 | 登录/登出/刷新/验证/自动登录 |
| Users | 5 | 18 | CRUD/批量/重置密码/状态切换 |
| Patients | 2 | 11 | CRUD/导入导出/软删除恢复 |
| Medical Cases | 1 | 5 | CRUD/列表搜索 |
| Medical Case Workflow | 1 | 5 | 状态流转/关闭/挂起/取消 |
| Medical Case Audit | 1 | 2 | 权限/审计日志 |
| Medical Case Print | 1 | 2 | 打印记录/打印日志 |
| Herbs | 3 | 13 | CRUD/批量操作/导入导出 |
| Formulas | 2 | 8 | CRUD/批量操作/验证 |
| Sync | 1 | 6 | 同步全流程 |
| Registrations | 1 | 6 | 挂号全流程 |
| Diagnostics | 1 | 5 | 日志管理+权限测试 |
| Health | 1 | 4 | 健康检查 |
| **通用/错误** | 5 | 18 | 认证/权限/验证/资源/边界 |
| **合计** | **30** | **119** | - |
