...
版本号的完整规则——**单一版本源、升档门槛、递增规则、发布一致性、机器门禁、禁止行为**——由 **[05-versioning-standard.md](../00-governance/05-versioning-standard.md)** 定义并强制执行，本节不重复其内容。发布前必须逐项核对该标准 §4 的一致性校验（tag == VersionPrefix、清单版本、SHA256SUMS 对照）。

发布流程视角只保留两条要点：

- 当前 **`0.0.x`** 阶段（未达 0.1.x 门槛，见标准 §2）。
- `Directory.Build.props` 的 `<VersionPrefix>` 为唯一版本源；打包省略 `-Version` 即取该值。
