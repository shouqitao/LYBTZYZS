...
中医诊所采用双模式架构 (远程 SQL Server + 本地 LocalDB)，医生外出看诊时使用本地模式离线工作，返回诊所后需要将本地产生的数据 (药材调价、新增患者、新建医案等) 同步到远程服务端。缺乏可靠的数据同步机制意味着: 离线数据孤立在本地设备上，无法汇入诊所统一数据库; 两端数据不一致导致药材库存价格错乱、患者信息重复; 医案记录分散在不同设备无法统一归档。
...
数据同步模块实现本地模式 (LocalDB) 与远程服务端 (SQL Server) 之间的双向数据同步。基于 SHA256 Checksum 比对差异，支持冲突手动解决。
...
1. **检查本地未完成医案 (SYNC-D01 前置约束)**:
   - 查询本地 LocalDB 中 CaseStatus = Active 或 Suspended 的医案数量
...
| 要求 | 说明 |
|------|------|
| 传输安全 | 远程同步使用 HTTPS，Bearer Token 认证 |
| 数据完整性 | SHA256 Checksum 校验，防止传输篡改 |
| 操作审计 | 同步操作记录到 SecurityAuditLog (时间+方向+记录数) |
| 冲突处理 | 不自动合并，人工裁决确保数据安全 |
...
| 操作 | 目标 | 说明 |
|------|------|------|
| 全量同步 | < 30s | 5000 患者 + 10000 医案 |
| 增量同步 | < 5s | Checksum 比对 + 差异传输 |
| Checksum 计算 | < 100ms/表 | SHA256 JSON 序列化 |

### 数据量估算

| 表 | 预估 | 说明 |
|------|------|------|
| 同步元数据 | ~100 KB | Checksum+时间戳，每表一条 |
| 传输数据量 | < 5 MB/次 | 增量同步典型负载 |
