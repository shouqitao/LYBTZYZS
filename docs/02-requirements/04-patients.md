...
- [ ] 手机号已存在 → 返回 409 "手机号 {PhoneNumber} 已存在"
...
| 属性 | 规格 |
|------|------|
| 搜索字段 | 姓名、拼音码、身份证号(模糊)、手机号(模糊) |
| 表格列 | 姓名、性别、年龄、手机号、身份证后四位、最后就诊时间、状态 |
| 筛选 | 状态筛选 (活跃/已禁用) |
| 工具栏按钮 | 新增患者、导入、批量删除 |
| 敏感字段 | 身份证号默认显示后四位，悬停显示完整 (需权限) |
| 行双击 | 打开患者详情 → 医案列表视图 |
...
| | 性别 | ComboBox (男/女) | 是 |
| | 出生日期 | DatePicker | 是 |
| | 身份证号 | TextBox (含格式校验) | 是 |
| | 证件类型 | ComboBox (身份证/护照/其他) | 否 |
| 联系方式 | 手机号 | TextBox (手机格式校验) | 是 |
| | 地址 | TextBox (长字段，跨两列) | 否 |
| | 紧急联系人 | TextBox | 否 |
| | 紧急联系电话 | TextBox | 否 |
| 医疗信息 | 婚姻状况 | ComboBox (未婚/已婚/离异/丧偶) | 否 |
| | 血型 | ComboBox (A/B/O/AB/未知) | 否 |
| | 过敏史 | TextBox (多行) | 否 |
| | 既往病史 | TextBox (多行) | 否 |
| | 备注 | TextBox (多行) | 否 |

#### 患者导入向导
遵循 18-ui-patterns.md §3.2 导入对话框规范:
1. 选择文件 (支持 .xlsx/.xls)
2. 字段映射 (自动匹配列名，手动调整)
3. 数据预览 (标注重复项、校验失败项)
4. 重复策略选择 (跳过重复)
...
| 维度 | 估算 |
|------|------|
| Patients | 1000-5000/yr, 5yr ~10000, ~2KB each |
...
| 表 | 预估 | 说明 |
|------|------|------|
| Patients | 1000-5000 条/年 | 新增+历史累积 |
| 5 年累积 | ~10,000 条 | 需分页查询支持 |
| 单条大小 | ~2 KB | 含地址/病史文本 |
...
| OQ-PAT-03 | 批量导入是否支持身份证号查重 (更新已有记录)? | **已解决**: 导入时检查身份证号重复 (DB + 批次内 HashSet)，重复项跳过 |
