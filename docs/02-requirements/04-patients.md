...
2. **身份证号唯一约束**：创建时校验身份证号（IdCardNumber）是否已存在，重复返回 409（ERR-20002）。同样适用于批量导入（行内互查 + 与系统已有患者比对）
3. 拼音由拼音服务自动生成，无需客户端提供
4. 年龄字段为计算字段，Mapperly 忽略，由控制器手动赋值
5. 新患者默认 `IsEnabled=true`、`IsDeleted=false`
...
1. **Doctor 所有权限制**：Doctor 角色仅可操作自己创建的患者（`CreatedBy = CurrentUserId`），Admin 可操作全部（US-PAT-004/005/006 通用规则）
2. 更新时重新生成 PinyinAbbreviation（姓名可能变更）
3. 更新时重新计算 Age（BirthDate 可能变更）
4. 电话唯一约束同样适用于更新
5. **身份证号唯一约束同样适用于更新**：修改身份证号时校验是否与系统其他患者重复，重复返回 409（ERR-20002）
...
1. **Doctor 所有权限制**：Doctor 角色仅可操作自己创建的患者（`CreatedBy = CurrentUserId`），Admin 可操作全部（通用规则，见 US-PAT-004）
2. 引用检查：查询 MedicalCase 表是否存在该患者的记录
3. 被引用的患者不可删除（保护医案完整性），返回 422
4. 软删除通过全局查询过滤器自动隐藏
5. **软删除（模块规则）**：通过 `IsDeleted` + 全局查询过滤器实现，默认隐藏已删除患者
...
1. **Doctor 所有权限制**：Doctor 角色仅可操作自己创建的患者（通用规则，见 US-PAT-004）
2. 禁用与软删除语义不同：禁用保留记录但对非管理员隐藏；软删除则视为已移除
3. 禁用不影响历史医案的访问（医生仍需查看已接诊患者的历史）
4. **禁用保护规则**（状态切换保护）：
   - 禁用前检查：该患者是否有 Active/Suspended 医案（未完成医案），有则拒绝禁用（返回 422 ERR-20005）
   - 禁用后效果：禁止为该患者创建新医案；已有历史医案仍可查阅，`PatientName` 按角色脱敏
   - 启用后效果：上述限制解除，恢复正常操作
