...
|------|------|
| 入口 | 管理模式 → 系统健康 |
| 整体状态 | 顶部大图标 + 文字: Healthy(绿✓)/Degraded(黄!)/Unhealthy(红✗) |
| 状态卡片 | 网格布局，每张卡片显示一个组件健康状态 |
| 卡片内容 | 组件名、状态指示灯、响应时间、最后检查时间 |
| 组件列表 | API 连接、数据库连接、磁盘空间、同步服务 |

#### 状态栏指示灯 (全局)
| 属性 | 规格 |
|------|------|
| 位置 | 主窗口底部状态栏左侧 |
| 显示 | 圆形指示灯 (绿/黄/红) + 文字 ("已连接"/"延迟高"/"连接断开") |
| 点击 | 弹出健康详情浮层 (最近一次检查结果) |
| 刷新 | 遵循配置的检查间隔 (默认 30s) |
...
| 要求 | 说明 |
|------|------|
| 端点保护 | 健康检查 `/health` 匿名可访问，诊断详情需 Admin+ |
| 信息隔离 | 公开端点仅返回 Healthy/Degraded/Unhealthy，不暴露内部架构 |
...
| 指标 | 预估 | 说明 |
|------|------|------|
| 健康检查记录 | 不持久化 | 实时查询，无存储 |
| 诊断日志 | 共用 SystemLog 表 | 同 14-logging.md 估算 |
