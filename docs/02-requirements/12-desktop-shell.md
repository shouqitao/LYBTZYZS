...
- 模块化架构需求: 系统包含 15 个功能模块 (9 业务模块 + 6 基础设施模块)，需要统一的加载和生命周期管理
...
应用启动 → Splash Screen (进度条) → StartupPipeline 执行步骤
  → [成功] 关闭 Splash → 显示登录页 → 认证 → 切换本地模式(如需) → 加载角色模块 → 导航到角色首页
...
|----------|------|
| API 不可达 | 提示 "服务器连接失败"，提供 "切换到本地模式" 按钮 |
| LocalDB 初始化失败 | 提示错误详情，提供 "重试" / "退出" 按钮 |
...
**Business Rules:**
1. 查看当前登录用户信息
2. 修改密码入口 (关联 users.md US-USER-009)
3. 个人资料编辑: 显示名称 / 电话 / 邮箱 (关联 users.md US-USER-010)
...
| 要求 | 说明 |
|------|------|
| 菜单强制 | 菜单可见性由后端角色决定，前端不可绕过 |
| 单实例 | Mutex 防止多开，避免数据竞争 |
| 安全启动 | 启动时验证 Token 有效性，过期强制重新登录 |
...
| 阶段 | 目标 | 说明 |
|-------|------|------|
| 冷启动 | < 5s | 含模块加载 + API 健康检查 |
| 模块加载 | < 500ms/模块 | Prism IModule.Initialize |
| 健康检查超时 | 3s | 超时降级为离线模式 |
