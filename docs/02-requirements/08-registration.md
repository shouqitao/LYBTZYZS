...
> **用户速览**：挂号 = 患者的「排队票」。前台帮患者挂个号，患者去排队等医生叫号；医生看到队列里有人，点「开始就诊」就行。
>
> | 我是… | 我能… |
> |-------|-------|
> | 前台 | 帮患者挂号、取消挂号、查看排队 |
...
**角色**: 前台接待员
**优先级**: Must
**状态**: ⚠️ 部分实现（权限已改 DoctorOrReceptionist✅；患者校验/同日重复/挂号费带出缺失）
...
**角色**: 前台接待员、医生、管理员
**优先级**: Must
**状态**: ⚠️ 部分实现（队列端点✅；当天过滤 REG-BR-012 缺失）
