...
|------|------|
| 默认视图 | 今日挂号队列 (按挂号时间排序) |
| 表格列 | 序号、挂号时间、患者姓名、医生、状态(Waiting/InProgress/Completed/Cancelled)、来源 |
| 状态着色 | Waiting=黄色, InProgress=蓝色, Completed=灰色, Cancelled=红色 |
| 筛选 | 日期选择器 (默认今日)、医生筛选、状态筛选 |
| 自动刷新 | 每 30s 自动刷新队列 (仅 Waiting 状态变化时更新) |
| 工具栏按钮 | 新增挂号、刷新(手动) |

#### 新增挂号表单 (对话框)
| 字段 | 控件 | 必填 | 校验规则 |
|------|------|------|----------|
| 患者 | 搜索 ComboBox (姓名/拼音码/身份证号) | 是 | 必须选择已有患者 |
| 快速新建患者 | 链接按钮 (打开患者新建子表单) | - | - |
| 就诊医生 | ComboBox (Doctor 角色) | 是 | 默认当前登录医生 |
| 挂号来源 | ComboBox (前台/医生) | 否 | 默认前台 |
| 备注 | TextBox (多行) | 否 | 最大 200 字符 |

#### 读卡快捷操作
- 挂号表单中身份证号字段旁显示"读卡"按钮
- 读卡成功自动填充: 姓名、性别、出生日期、身份证号、地址
- 自动匹配已有患者记录，匹配到则直接选中
- 未匹配到则提示"是否新建患者?"，确认后自动填入读卡数据
