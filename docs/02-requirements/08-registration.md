...
> 双模式工作流（远程/本地分野）设计见 [ADR-0013（SignalR 实时推送）](../03-architecture/decisions/0013-signalr-real-time.md)。此处的「双模式」指**部署模式**（远程/本地）下的工作流分野，区别于「两源模型」——后者描述同一远程模式内按 `Source` 字段的来源区分。
