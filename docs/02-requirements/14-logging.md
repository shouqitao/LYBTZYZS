...
|------|------|
| 入口 | 管理模式 → 系统健康 → 日志查看 |
| 筛选 | 时间范围(日期选择器)、级别(Error/Warning/Info/Debug)、模块、搜索关键词 |
| 表格列 | 时间、级别(着色)、模块、消息、用户 |
| 级别着色 | Error=红, Warning=黄, Info=蓝, Debug=灰 |
| 详情 | 行展开显示完整消息 + 属性 (Properties JSON) |
| 分页 | 遵循 18-ui-patterns.md §1.3 分页规范 |
| 导出 | 导出为文本文件按钮 (仅当前筛选结果) |

#### 审计日志查看器 (Admin+)
| 属性 | 规格 |
|------|------|
| 入口 | 管理模式 → 系统健康 → 审计日志 |
| 筛选 | 时间范围、操作类型(CRUD)、用户、实体类型、实体 ID |
| 表格列 | 时间、用户、操作、实体类型、变更摘要、IP 地址 |
| 变更详情 | 行展开显示变更前后对比 (旧值 → 新值) |
...
| 要求 | 说明 |
|------|------|
| 日志访问 | 日志查看器仅 Admin+ 可访问 |
| 内容脱敏 | 日志写入时自动脱敏: 手机号(中间4位*)、身份证号(保留前3后4) |
| 审计完整性 | SecurityAuditLog 不可编辑/删除，仅追加写入 |
| 日志保留 | 默认保留 365 天，超期自动清理 |
...
| 指标 | 目标 | 说明 |
|------|------|------|
| 日志写入 | < 5ms/条 | 异步 Serilog File Sink |
| 审计日志查询 | < 500ms | 含分页和筛选 |
