...
**角色**: 开发人员/运维人员
**优先级**: Should
**状态**: ✅ 已实现（2026-08-13 审计确认：Serilog 异常链已含 InnerException——`--->` 多行渲染；本 US 记录定位方法）

**作为** 开发人员，**我想要** 一份明确的错误日志定位工作流，**以便** 遇到 500 时按步骤快速定位根因（而非逐层猜）。

**验收标准**:
- [ ] 文档化定位步骤：①启动首行版本 → ②CorrelationId 关联 → ③异常多行看 `--->` 内层 → ④当前日志文件
- [ ] 明确「日志多行结构」：ERR 行摘要 + 后续异常堆栈（含 `--->` 内层异常链）——grep 时需 `-A` 多行
- [ ] 明确「生产屏蔽不破坏日志诊断」：响应体屏蔽堆栈，日志仍含完整异常链

**业务规则**:
1. Serilog 渲染异常链：`LogError(ex, ...)` 自动输出 `--->` 内层（@x 字段）——**不需要额外代码**
2. 常见误区：只 grep ERR 单行看不到内层——需 `grep -A` 或看完整异常块
3. 定位三步：版本（代码新旧）→ CorrelationId（跨模块）→ 内层异常（根因）

**双模式**: 远程/本地一致

**实现参考**: 日志文件多行异常结构（webapi.log 实证）、docs/06-operations 定位指南（2026-08-13 补）
...
| v1.1 | 2026-08-13 | 新增 US-ERR-009 定位工作流（审计确认 InnerException 已有——Serilog @x 渲染） | 真机测试审计：初判缺 InnerException，实证为定位方法问题 |
