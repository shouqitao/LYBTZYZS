...
> ⚠️ 本文档超 400 行（444 行），建议拆分。

> 验方（经验方/Formula）是中医诊所在长期临床实践中积累的固定验方模板，由若干味药材按特定剂量、煎法组成。模块负责验方模板的创建、编辑、共享、验证和批量导入导出，核心特色是**延迟绑定 + 验证工作流**——导入验方的药材名称可暂不关联系统药材库，后续通过验证端点逐个绑定；所有药材完成验证后自动晋升 `Validated`，已验证验方药材被修改则自动降级回 `Draft`（FLAW-F1）。
>
> | 我是… | 我能… |
> | ------- | ------- |
> | 医生 | 查看自己和共享的验方、创建/编辑自己的验方、导入验方到处方 |
> | 管理员 | 全部操作：管理所有验方、批量导入、启用/禁用 |
> | 前台 | ❌ 不涉及验方（2026-08-03 决策） |
...
7. **记录-Only 模式**: 药材不管理库存，仅维护基础信息（详见 [05-herbs.md](05-herbs.md) 模块级业务规则「记录模式」）。
...
**角色**: 管理员
**优先级**: Must
**状态**: ✅ 已实现（toggle-status + batch-enable/disable）
...
**角色**: 管理员
**优先级**: Should
**状态**: ✅ 已实现（T4: export/import-template 已补；空列表 400 ✅；2026-08-19 P1：模板示例/DTO 对齐完成；2026-08-19 P2：导出含药材组成明细 + 分类筛选参数对齐完成——`GET /formulas/export?category=` + `FormulaDetailDto` Herbs 明细）

**作为** 管理员，**我想要** 批量操作验方、导出 JSON 和下载导入模板，**以便** 高效管理验方数据并备份数据。

**验收标准**:

- [ ] 批量删除/启用/禁用 → 单次 SaveChanges，返回成功/失败计数
- [ ] 导出 JSON 数组 → 每行验方包含药材组成详情（2026-08-13：Excel→JSON——后端不涉及 Excel，保持通用性）
- [ ] 下载模板 → 返回 JSON 模板（字段说明 + 示例 + 必填）
- [ ] 空列表请求 → 返回 400
