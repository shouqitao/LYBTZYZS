...
|------|------|
| 搜索字段 | 用户名、姓名、拼音码 |
| 表格列 | 用户名、姓名、角色、状态(启用/禁用)、最后登录时间 |
| 筛选 | 角色下拉筛选、状态下拉筛选 |
| 工具栏按钮 | 新增用户、批量删除(选中后显示) |

#### 用户新增/编辑表单
| 字段 | 控件 | 必填 | 校验规则 |
|------|------|------|----------|
| 用户名 | TextBox | 是 | 3-20 字符，仅字母数字下划线 |
| 姓名 | TextBox | 是 | 2-10 字符 |
| 密码 (仅新增) | PasswordBox | 是 | 6-20 字符，含大小写+数字 |
| 角色 | ComboBox (Doctor/Admin/SuperAdmin) | 是 | - |
| 状态 | ToggleSwitch | 否 | 默认启用 |
...
1. 创建用户 → 分配角色 + 初始密码
2. 首次登录 → 强制修改密码 (v2.0)
3. 日常使用 → Token 认证 + 权限校验
4. 禁用用户 → 吊销 Token + 阻止登录
5. 重置密码 → 生成临时密码 → 通知用户
...
| 表 | 预估 | 说明 |
|------|------|------|
| Users | 5-15 条 | 诊所员工，极少变化 |
| AuditLog | ~200 条/月 | 用户 CRUD 操作 |
