...
1. 修改 appsettings.json → 2. 验证 JSON 格式 (启动时自动校验) → 3. 重启客户端生效 → 4. 验证日志确认配置已加载

> 注意: 仅 Logging 配置支持热更新（修改后自动生效），其他所有配置变更均需重启。
...
|------|------|
| 布局 | 左侧导航树 + 右侧设置面板 |
| 导航分类 | 连接设置、功能开关、日志配置、打印设置 |

#### 连接设置面板
| 字段 | 控件 | 说明 |
|------|------|------|
| API 地址 | TextBox | 远程服务器 URL |
| 连接测试 | 按钮 | 点击后显示连接状态 (绿/红 + 延迟 ms) |
| 运行模式 | ToggleSwitch (远程/本地) | 切换需重启 |

#### 功能开关面板
| 属性 | 规格 |
|------|------|
| 布局 | 开关列表 (名称 + 描述 + ToggleSwitch) |
| 分组 | 按模块分组显示 |
| 即时生效 | 仅 Logging 配置支持热加载，其他需重启提示 |

#### 配置变更流程
1. 修改 → 即时校验 (格式/范围) → 显示保存按钮
2. 保存 → 写入 appsettings.json
3. 需重启项显示"重启后生效"提示 (黄色 Toast)
...
| 要求 | 说明 |
|------|------|
| 访问控制 | 系统设置仅 SuperAdmin 可修改 |
| 配置完整性 | 启动时自动校验 JSON 格式，格式错误拒绝启动 |
| 敏感配置 | 数据库连接字符串、JWT 密钥不明文显示在 UI 中 |
...
| 排除项 | 原因 |
|--------|------|
| 配置管理 UI (Web 界面修改配置) | 诊所规模小，appsettings.json + 环境变量足够，后续版本考虑 |
| FeatureToggle 热更新 | 当前阶段重启够用，热更新复杂度高，延期到运维完善 Sprint |
| 配置加密存储 (Data Protection API) | 当前通过环境变量隔离敏感信息，后续版本考虑 |
| 配置版本管理/回滚 | 运维通过 Git 管理配置文件变更，系统内不实现 |
| 集中配置中心 (Consul/etcd) | 单机部署场景不需要，后续版本多节点部署时考虑 |
...
| ID | 问题 | 状态 |
|----|------|------|
| OQ-CFG-01 | FeatureToggle 热更新何时实现? | 延期。当前阶段需重启 Desktop 生效，计划在运维完善 Sprint 实现 |
| OQ-CFG-02 | 是否需要配置管理 UI (Web 界面)? | 延期。当前 appsettings.json 足够，后续版本根据运维反馈决定 |
...
| 2026-02-21 | FeatureToggle 热更新支持延期实现 | 当前阶段重启够用，热更新复杂度高 | CFG-07 |
