...
| 属性 | 规格 |
|------|------|
| 业务错误 | Toast 提示 (黄色，5s 自动消失)，显示用户友好消息 |
| 验证错误 | 字段下方红色文字 (具体字段) + 表单顶部错误摘要 |
| 未找到/冲突 | Toast 提示 (黄色)，含错误码 |
| 系统错误 | 对话框: 标题"操作失败"，内容含错误码 + 友好消息 + "如问题持续请联系管理员" |
| 网络错误 | 对话框: 标题"网络连接失败"，含重试按钮 + 切换本地模式提示 |
| 未授权 | 自动跳转登录页 + Toast "登录已过期，请重新登录" |

#### 错误代码展示
- 用户可见错误码格式: ERR-XXXXX
- 点击错误码可展开详细信息 (仅 Admin+ 角色)
- 详细信息含: 时间戳、请求路径、堆栈摘要 (脱敏)
...
| 要求 | 说明 |
|------|------|
| 信息脱敏 | 生产环境错误响应不包含堆栈跟踪、SQL 语句等内部信息 |
| 错误码追踪 | 系统错误返回追踪 ID (TraceId)，不暴露内部实现 |
| 日志脱敏 | 异常日志中 PatientModel 等敏感实体自动应用 [SensitiveData] 掩码 |
...
| 指标 | 目标 | 说明 |
|------|------|------|
| 异常处理开销 | < 5ms | BusinessException→HTTP 响应 |
| 全局异常日志 | < 10ms | 含序列化 + Serilog 写入 |

### 数据量估算

| 表 | 预估 | 说明 |
|------|------|------|
| SystemLog | ~500 条/月 | Error+Warning 级别 |
| 5 年累积 | ~30,000 条 | 需归档策略 |
