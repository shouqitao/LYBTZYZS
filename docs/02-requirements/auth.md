...
- **验收标准**:
  - [ ] 正确凭据返回 Token 和用户信息
  - [ ] 错误凭据返回 ErrorCode=InvalidCredentials(101)
  - [ ] 禁用用户无法登录
  - [ ] 触发限流后返回 429
...
- **本地模式**: 不适用
- **验收标准**:
  - [ ] 重复使用 RefreshToken 触发整个 Family 失效
  - [ ] 客户端正确处理 TokenRevoked 错误
...
| 编号 | 问题 | 影响范围 | 状态 |
|------|------|----------|------|
| 1 | 本地模式下自动登录的实现方式 | FR-AUTH-002 | 待讨论 |
| 2 | 本地模式下的会话超时是否需要与远程一致 | FR-AUTH-006 | 待讨论 |
