...
中医诊所管理系统需在小型诊所（1-5 台终端）环境下提供稳定、安全、高效的诊疗服务支持。系统面对的核心挑战包括：网络不稳定环境下的业务连续性、患者敏感信息的隐私保护、以及中医处方打印的合规性要求。

## Success Metrics (v1.0)

1. **核心诊疗流程完成率 > 99.9%** — 挂号→看诊→开方→打印全链路零中断
2. **API 响应时间 P95 < 200ms** — 简单查询 (单实体 CRUD) 级别
3. **零数据丢失** — 同步冲突可检测+可解决，RPO=24h 内无业务数据丢失
4. **患者敏感字段访问均有审计记录** — SecurityAuditLog 100% 覆盖认证事件
...
> **[已修订 2026-02-21]** 响应时间 SLA 标注为待实测校准，当前目标值为预估值，需实际负载测试后确认
> 原因: SLA 需实测确定，PRD 预估值仅供参考  |  参考: NFR-08
...
- **验收标准**:
  - [x] 在标准数据量 (患者 5000 + 医案 25000) 下，上述 P95 指标达标 (2026-03-10 基准测试通过，详见 `docs/05-development/09-performance-baseline.md`)
  - [ ] 慢查询监控 (SlowQueryThresholdMs=1000) 在生产环境启用

> **[已修订 2026-02-21]** 其他 NFR 细节要求简化，启动时间、页面切换等具体数值允许合理偏差
> 原因: 非功能性细节，具体指标需实测校准  |  参考: NFR-09
...
> **[已修订 2026-02-21]** 并发连接数配置要求放宽，MaxConnections 等参数允许运行时通过配置文件调整
> 原因: 可运行时调整，不需要 PRD 硬性规定具体数值  |  参考: NFR-07
...
- **速率限制**: v1.0 已定义配置但未启用 (当前阶段)，计划在生产部署时启用
...
> **注**: 安全需求按模块分布定义，各模块 PRD 的 `安全要求` 小节包含具体的访问控制、审计和隔离规则。跨模块安全策略参见 `10-sensitive-data.md`（敏感数据分级）和 `08-authorization-policies.md`（授权策略）。
...
> v1.0 不实现密钥轮换 (Key Rotation)。如需轮换，需要解密全部数据 → 生成新密钥 → 重新加密，复杂度较高，待后续版本评估。
