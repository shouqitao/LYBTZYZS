...
| 属性 | 规格 |
|------|------|
| 布局 | 居中卡片式登录框，背景为诊所 Logo + 渐变色 |
| 字段 | 用户名 (TextBox)、密码 (PasswordBox，含显示/隐藏切换) |
| 登录按钮 | 宽度 100%，主色按钮，显示加载动画 |
| 错误提示 | 字段下方红色文字，登录失败 Toast 提示 (含错误码) |
| 记住登录 | 复选框，勾选后 Token 持久化到本地 |
| 连接状态 | 登录框下方显示 API 连接状态指示灯 (绿/红/灰) |

#### 自动登录
- 应用启动时检测本地 Token，有效则自动跳转主界面
- Token 过期时尝试 Refresh，失败则跳转登录页
- 同时只允许一个客户端实例运行 (Mutex 检测)
...
| 维度 | 估算 |
|------|------|
| SecurityAuditLog | ~1000/mo |
| RefreshToken | <20 active |
...
| 表 | 预估 | 说明 |
|------|------|------|
| SecurityAuditLog | ~1000 条/月 | 登录+登出+敏感操作 |
| RefreshToken | < 20 活跃 | 同时在线终端数 |
...
> 认证错误码归入用户模块命名空间 (1xxxx)，与 [users.md](03-users.md) 统一编号体系。
