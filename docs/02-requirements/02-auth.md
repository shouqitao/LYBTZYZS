...
**角色**: 系统安全
**优先级**: Must
**状态**: ⚠️ 部分实现（会话级旋转+重放检测已实现；FamilyId 族撤销设计废弃，重放不撤销整族）
...
**角色**: 系统安全
**优先级**: Should
**状态**: ✅ 已实现（T4: AutoLoginToken 服务端签发+轮换）
...
**角色**: 系统管理员
**优先级**: Must
**状态**: ✅ 已实现（T4: UserReservedNameHelper 保留用户名清单）
...
**角色**: 系统安全
**优先级**: Must
**状态**: ✅ 已实现（本地 Login FixedWindow 5/min）
