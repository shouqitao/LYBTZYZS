...
| 维度 | 估算 |
|------|------|
| MedicalCases | 2000-5000/yr, 5yr ~15000, ~3KB each |
...
| 操作 | 目标 | 说明 |
|------|------|------|
| 医案创建 | < 200ms | 含处方+诊断聚合保存 |
| 医案搜索 | < 300ms | 多条件搜索 (患者/医生/日期) |
| 处方打印渲染 | < 2s | FixedDocument 生成 |
| 医案列表 | < 200ms | 含分页 |
...
| 表 | 预估 | 说明 |
|------|------|------|
| MedicalCases | 2000-5000 条/年 | 每诊 10-20 例 |
| 5 年累积 | ~15,000 条 | 最大表，需索引优化 |
| 单条大小 | ~3 KB | 含诊断文本+处方明细 |
...
| 值 | 名称 | 说明 | 打印标注 |
|----|------|------|---------|
| Default (0) | 水煎 | 默认煎法，不单独标注 | (无) |
| PreDecoct (1) | 先煎 | 矿石/贝壳类，需先煎 20-30 分钟 | 先煎 |
| PostAdd (2) | 后下 | 芳香类，最后 5-10 分钟下 | 后下 |
| MeltIn (3) | 烊化 | 胶类药材，热药液中融化 | 烊化 |
| TakeWithWater (4) | 冲服 | 粉末/液体类，药液冲服 | 冲服 |
| WrapDecoct (5) | 包煎 | 细小种子/绒毛类，纱布包裹煎 | 包煎 |
| SeparateDecoct (6) | 另炖 | 贵重药材，单独炖煮 | 另炖 |
