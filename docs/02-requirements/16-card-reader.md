...
| 属性 | 规格 |
|------|------|
| 触发位置 | 患者表单身份证号字段旁"读卡"按钮 |
| 读卡状态 | 按钮显示加载动画 → 成功(绿✓ 2s) → 自动填充表单 |
| 读取超时 | 10s 超时，显示"读取超时，请重试" Toast |
| 读卡失败 | 按钮恢复原始状态 + Toast 显示错误信息 (含错误码) |
...
| 姓名 | 患者姓名 | 直接填充 |
| 性别 | 性别选择 | 男/女自动选择 |
| 出生日期 | 出生日期 | 格式转换 yyyyMMdd → DateTime |
| 公民身份号码 | 身份证号 | 直接填充 + 格式校验 |
| 住址 | 地址 | 直接填充 |
...
| 位置 | 患者表单身份证号字段下方 |
| 已连接 | 绿色"读卡器已连接"文字 |
| 未连接 | 灰色"读卡器未连接"文字 + 重试链接 |
| 读取中 | 蓝色加载动画 + "请放置身份证..." 文字 |
