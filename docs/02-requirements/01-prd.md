...
| 6 | 医案管理 (MedicalCases) | 19 | 聚合根。诊断 + 处方 + 打印完整生命周期，CQRS 模式，复用上次处方（D6） |
| 7 | 挂号管理 (Registration) | 8 | 前台排队 + 医生快速就诊，医案联动回写，SignalR 实时推送（R10） |
| 8 | 处方打印 (Printing) | 4 | A5/A4 模板、PDF 导出、打印回写服务器 |
| 9 | 报表管理 (Reports) | 3 | 收入/就诊统计/药材使用排行，按时间范围查询（A7） |
| 10 | 平台基础设施 (Platform) | 43 | Shell + Config + Error + Logging + Health + CardReader（含 SHELL-010~019 中 v1.0 的 8 项） |
| **合计** | | **141** | |
