...
**经验方流失**：中医医生在长期诊疗中积累的经验方（验方）缺乏系统化手段。验方的药材组成、剂量配比依赖个人记忆与纸质笔记，难以在团队间共享和传承；人员离职即造成知识断层。
...
| # | 模块 | US 数 | 核心能力 |
|---|------|-------|---------|
| 1 | 认证与会话 (Auth) | 14 | JWT 登录/登出、Token 旋转、重放攻击检测、双模式认证、保留用户名、AutoLoginToken |
| 2 | 用户管理 (Users) | 12 | CRUD、四级权限体系、密码策略、账户锁定、批量操作、层级约束 |
| 3 | 患者管理 (Patients) | 14 | 档案 CRUD、拼音码搜索、身份证读卡、导入导出、敏感数据脱敏、电话唯一、引用检查 |
| 4 | 药材管理 (Herbs) | 13 | 药材库 CRUD、分类管理、拼音检索、批量导入、引用检查（含单删） |
| 5 | 验方管理 (Formulas) | 14 | 经验方 CRUD、Draft↔Validated 状态机、药材绑定验证、共享机制、克隆 |
| 6 | 医案管理 (MedicalCases) | 20 | 聚合根。诊断 + 处方 + 打印完整生命周期，CQRS 模式，历史聚合/批量详情、EditReason、审计 diff |
