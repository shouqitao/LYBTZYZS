...
| 排除项 | 原因 |
|--------|------|
| 多窗口/多标签页 | 增加架构复杂度，诊所单屏场景不需要，v2.0+ 考虑 |
| 网络状态实时监控 | 状态栏网络指示器延期到 v2.0 |
| 前进导航 | 增加导航复杂度，仅后退已满足需求 |
| 插件市场/动态模块下载 | 超出 v1.0 范围，模块编译时静态注册 |
| 最后登录时间/IP 信息显示 | 非 MVP 必要，Sprint 后续实现 |
...
| OQ-SHELL-02 | 启动诊断报告是否暴露给用户查看? 还是仅写入日志? | 延期。当前仅日志记录，v2.0 考虑管理员诊断面板 |
| OQ-SHELL-03 | 本地模式下哪些菜单项应禁用? 需要完整的本地模式菜单可用性矩阵 | 待定。当前标注 "部分菜单不可用"，需逐项确认 |
| OQ-SHELL-04 | 状态栏是否需要显示网络状态指示? | 延期到 v2.0 |
...
| 2026-02-21 | 缺少最后登录时间/IP 信息显示 | 非 MVP 必要，延期到 Sprint 后续 | SHELL-08 |
