...
| 要求 | 说明 |
|------|------|
| 打印审计 | 每次打印记录 MedicalCasePrintLog (操作人+时间) |
| 打印后保护 | 已打印医案不可删除，编辑需填写原因 (ERR-30403/30404) |
| 重打限制 | 仅 Admin+ 可授权重新打印 |

### 打印工作流
1. 医案完成 → 弹出"是否打印处方?"提示
2. 确认打印 → 渲染 FixedDocument → 系统打印对话框
3. 打印成功 → 记录 PrintLog + 设置 IsPrinted=true
4. 后续编辑 → 强制填写编辑原因 + 审计记录
5. 重新打印 → 仅 Admin+ 授权 (记录原因)
...
| 表 | 预估 | 说明 |
|------|------|------|
| MedicalCasePrintLog | ~3000 条/年 | 每个打印医案一条 |
...
| Configuration (configuration.md) | US-CFG-002 ClinicSettings (诊所名称/科别/地址/电话) |
| Prescription (medical-cases.md) | 处方药材列表、费用计算规则 (US-MC-004) |
