...
> **用户速览**：打印 = 把处方变成纸质处方笺。医生开完方子，点打印，患者拿着处方去取药。
>
> | 我是… | 我能… |
> |-------|-------|
> | 医生 | 打印处方（A5/A4）、预览、导出 PDF |
> | 管理员 | 查看打印记录 |
...
**角色**: 医生（2026-08-03 决策：仅 Doctor 打印；管理员只可查看打印记录）
**优先级**: Must
**状态**: ✅ 已实现

> **权限说明（2026-08-03 决策）**：处方打印仅 Doctor（`DoctorOnly` 策略待补，见 04-permissions P1-6）；管理员可查看打印记录（复用医案审计/打印日志查询权限）。
