...
**角色**: sysadmin
**优先级**: Should
**状态**: ✅ 已实现（2026-09-24 校准：`ICardReaderDiagnostics`/`CardReaderDiagnosticsService` 已存在并注册，8/8 AC 见矩阵 US-SHELL-019）
...
**关联**: US-HERB/US-FORM 批量导入、US-PAT 档案导入、US-SHELL-013 备份
...
**角色**: sysadmin
**优先级**: Should
**状态**: ✅ 已实现（2026-09-24 交付 `docs/06-operations/13-go-live-checklist.md`——清单/双轨/健康检查三缺口补齐；回滚方案 `08-deployment-rollback.md` 既有）

**作为** 系统运维，**我想要** 按检查清单完成上线并具备回滚能力，**以便** 业务连续、故障可恢复。

**验收标准**:

- [x] 上线检查清单（数据库备份/配置校验/连通性/角色账号就绪）
- [x] 试运行期支持双轨运行（新系统 + 原流程并行观察）
- [x] 回滚方案：备份恢复（US-SHELL-013）+ 版本回退（Velopack）
- [x] 上线后健康检查（HealthController /health）
...
**角色**: 所有用户
**优先级**: Could
**状态**: ✅ 已实现（2026-09-24 交付 `training/faq.md` + `training/support-process.md` + `training/quick-start.md` 三角色快速上手；`clinician-training-guide.md` 医生详版既有）

**作为** 诊所员工（前台/医生），**我想要** 快速上手系统并遇到问题能自助解决，**以便** 不依赖外部支持完成日常诊疗。

**验收标准**:

- [x] 操作培训材料（各角色快速上手指南，≤30 分钟学完）
- [x] 常见问题手册（FAQ：登录/打印/读卡器/数据等高频问题）
- [x] 系统内帮助入口（F1 帮助已有——补充内容维护）
- [x] 支持流程（远程协助/工单/版本回退渠道）
...
- [ ] **Mutex 单实例**（Program.cs）：已有实例在跑 → 拒绝启动退出（防跨脚本双开）
