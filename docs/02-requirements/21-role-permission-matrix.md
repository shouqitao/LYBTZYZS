...
## Purpose
角色权限矩阵是系统授权的单一事实来源 (Single Source of Truth)，定义每个角色对每个资源的操作权限。各模块 PRD 的权限描述以此文档为准。

## 目标用户
- 开发团队: 实现 Authorization Policy 时参考
- 测试团队: 设计权限测试用例
- 安全审计: 验证最小权限原则

## 成功标准
1) 每个模块的每个操作都有明确的角色授权
2) 权限矩阵与代码中 PolicyConstants 一致
3) 无权限遗漏或冲突

## Out of Scope
- UI 菜单可见性: 见 18-ui-patterns.md §4.3
- 数据级权限 (如个人验方隔离): 见各模块 PRD
- v2.0 权限扩展 (如科室级权限)
