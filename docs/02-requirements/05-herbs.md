...
|------|------|
| 搜索字段 | 名称、拼音码、分类 |
| 表格列 | 名称、拼音码、分类、单价、单位、库存状态(红/绿)、启用状态 |
| 筛选 | 分类下拉筛选 (解表药/清热药/... 共 21 类)、状态下拉筛选 |
| 工具栏按钮 | 新增药材、批量导入、批量删除 |
| 特殊 | 无分页 (药材总数 < 1000，一次性加载) |
...
| 单位 | ComboBox (克/片/粒/包/瓶) | 是 | - |
| 产地 | TextBox | 否 | 最大 50 字符 |
| 规格 | TextBox | 否 | 最大 50 字符 |
| 成本价 | DecimalUpDown | 否 | >= 0，管理员可见 |
| 用法 | TextBox | 否 | 煎服/冲服/外用等 |
...
| 要求 | 说明 |
|------|------|
| 访问控制 | 药材管理仅 Admin+ 可操作，Doctor/Receptionist 只读 |
| 价格保护 | 成本价仅 Admin/SuperAdmin 可见，Doctor 只看零售价 |
| 批量操作审计 | 批量导入/删除操作记录到 SecurityAuditLog |
...
| 维度 | 估算 |
|------|------|
| Herbs | 300-500 records, ~500B each, 21 categories |
...
| 表 | 预估 | 说明 |
|------|------|------|
| Herbs | 300-500 条 | 中药材目录，极少变化 |
| 分类 | 21 种 | 固定分类枚举 |
| 单条大小 | ~500 B | 名称+属性，无大文本 |
