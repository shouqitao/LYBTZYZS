...
public class ReportRepository : IReportRepository
{
    ...
    /// <inheritdoc/>
    public async Task<List<ReportDayValueDto>> GetRegistrationFeeByDayAsync(DateTime startDate, DateTime endDate, CancellationToken cancellationToken = default)
    {
        return await _context.Registrations
            .Where(r => !r.IsDeleted && r.CreatedAt >= startDate && r.CreatedAt < endDate.AddDays(1))
    ...
    /// <inheritdoc/>
    public async Task<List<ReportDayValueDto>> GetMedicineFeeByDayAsync(DateTime startDate, DateTime endDate, CancellationToken cancellationToken = default)
    {
        return await (
    ...
    /// <inheritdoc/>
    public async Task<List<ReportDayCountDto>> GetConsultationCountByDayAsync(DateTime startDate, DateTime endDate, CancellationToken cancellationToken = default)
    {
        return await _context.MedicalCases
            .Where(mc => !mc.IsDeleted && mc.CreatedAt >= startDate && mc.CreatedAt < endDate.AddDays(1) && mc.CaseStatus == MedicalCaseStatus.Completed)
    ...
    /// <inheritdoc/>
    public async Task<List<DoctorPerformancePointDto>> GetDoctorPerformanceAsync(DateTime startDate, DateTime endDate, CancellationToken cancellationToken = default)
    {
        var cases = _context.MedicalCases
            .Where(mc => !mc.IsDeleted && mc.CreatedAt >= startDate && mc.CreatedAt < endDate.AddDays(1) && mc.CaseStatus == MedicalCaseStatus.Completed);

        var consultationCounts = await cases
            .GroupBy(mc => mc.DoctorName)
            .Select(g => new { DoctorName = g.Key, Count = g.Count() })
            .ToListAsync(cancellationToken);

        var registrationFees = await (
            from mc in cases
            join r in _context.Registrations on mc.Id equals r.MedicalCaseId
            where !r.IsDeleted
            group r by mc.DoctorName into g
            select new { DoctorName = g.Key, Total = g.Sum(x => x.RegistrationFee) })
            .ToListAsync(cancellationToken);

        var medicineFees = await (
            from mc in cases
            join p in _context.Prescriptions on mc.Id equals p.MedicalCaseId
            where !p.IsDeleted
            join pi in _context.PrescriptionItems on p.Id equals pi.PrescriptionId
            group pi by mc.DoctorName into g
            select new { DoctorName = g.Key, Total = g.Sum(x => x.UnitPrice * x.Dosage) })
            .ToListAsync(cancellationToken);

        var prescriptionCounts = await (
            from mc in cases
            join p in _context.Prescriptions on mc.Id equals p.MedicalCaseId
            where !p.IsDeleted
            group p by mc.DoctorName into g
            select new { DoctorName = g.Key, Count = g.Count() })
            .ToListAsync(cancellationToken);

        return consultationCounts
            .Select(c => new DoctorPerformancePointDto(
                c.DoctorName,
                c.Count,
                registrationFees.FirstOrDefault(x => x.DoctorName == c.DoctorName)?.Total ?? 0,
                medicineFees.FirstOrDefault(x => x.DoctorName == c.DoctorName)?.Total ?? 0,
                prescriptionCounts.FirstOrDefault(x => x.DoctorName == c.DoctorName)?.Count ?? 0))
    ...
    /// <inheritdoc/>
    public async Task<List<PatientFlowPointDto>> GetPatientFlowByDayAsync(DateTime startDate, DateTime endDate, CancellationToken cancellationToken = default)
    {
        var patientDays = await _context.MedicalCases
            .Where(mc => !mc.IsDeleted && mc.CreatedAt >= startDate && mc.CreatedAt < endDate.AddDays(1) && mc.CaseStatus == MedicalCaseStatus.Completed)
        ...
        var firstDayByPatient = await _context.MedicalCases
            .Where(mc => !mc.IsDeleted && mc.CaseStatus == MedicalCaseStatus.Completed && patientIds.Contains(mc.PatientId))
