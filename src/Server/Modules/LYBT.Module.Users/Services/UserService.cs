    ...
    public class UserService : BaseService<User>, IUserService
    {
            ...
            CommonStatus? status = null)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Result<PagedResult<UserDto>>.Failure("获取用户列表失败");
            }
        }
        ...
        public async Task<Result<UserDto>> GetByIdAsync(Guid id)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Result<UserDto>.Failure("获取用户详情失败");
            }
        }
        ...
        /// 搜索用户（返回所有匹配结果）
        /// </summary>
        public async Task<Result<List<UserDto>>> SearchAsync(string keyword)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                return Result<List<UserDto>>.Failure("搜索用户失败");
            }
        }
        ...
        public async Task<Result<UserDto>> CreateAsync(UserInputDto dto, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (!CanManageUser(currentRole, dto.Role))
                {
                    ...
                    return Result<UserDto>.Failure($"您没有权限创建{roleName}账户");
                }
                ...
                if (reservedUsernames.Any(reserved => string.Equals(dto.UserName, reserved, StringComparison.OrdinalIgnoreCase)))
                {
                    ...
                    return Result<UserDto>.Failure($"用户名 '{dto.UserName}' 为系统保留用户名，不可使用");
                }
                ...
                if (existingUser)
                {
                    ...
                    return Result<UserDto>.Failure($"用户名 '{dto.UserName}' 已存在，请使用其他用户名");
                }
            ...
            catch (Exception ex)
            {
                ...
                return Result<UserDto>.Failure("创建用户失败");
            }
        }
        ...
        /// Issue #1909: 添加三角色权限控制
        /// </summary>
        public async Task<Result<UserDto>> UpdateAsync(Guid id, UserInputDto dto, CancellationToken cancellationToken = default)
        {
            try
            {
                var entity = await _repository.GetByIdAsync(id);
                ...
                if (!CanManageUser(currentRole, entity.Role))
                {
                    ...
                    return Result<UserDto>.Failure("您没有权限更新该用户");
                }
                ...
                if (dto.Role != entity.Role && !CanManageUser(currentRole, dto.Role))
                {
                    ...
                    return Result<UserDto>.Failure("您没有权限将用户角色修改为该级别");
                }
            ...
            catch (Exception ex)
            {
                ...
                return Result<UserDto>.Failure("更新用户失败");
            }
        }
        ...
        public async Task<Result<BatchOperationResultDto>> BatchDeleteAsync(List<Guid> ids)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Result<BatchOperationResultDto>.Failure("批量删除用户失败");
            }
        }
        ...
        /// 修改个人信息 (Issue #1888)
        /// </summary>
        public async Task<Result<UserDto>> ChangeProfileAsync(Guid userId, ChangeProfileDto dto)
        {
            try
            {
                // 验证输入
                if (dto == null)
                {
                    return Result<UserDto>.Failure("个人资料信息不能为空");
                }

                if (string.IsNullOrWhiteSpace(dto.RealName))
                {
                    return Result<UserDto>.Failure("真实姓名不能为空");
                }
            ...
            catch (Exception ex)
            {
                ...
                return Result<UserDto>.Failure("修改个人资料失败");
            }
        }
