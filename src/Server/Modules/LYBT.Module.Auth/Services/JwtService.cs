...
public class JwtService : IJwtService
{
    ...
    /// </summary>
    public Result<LoginResponse> RefreshToken(string expiredToken)
    {
        if (string.IsNullOrEmpty(expiredToken))
            return Result<LoginResponse>.Failure(ErrorCode.AuthTokenInvalid, "令牌不能为空");
        ...
        try
        {
            ...
            if (securityToken is not JwtSecurityToken jwtToken)
                return Result<LoginResponse>.Failure(ErrorCode.AuthTokenInvalid, "无效的令牌格式");
            ...
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(userName) || string.IsNullOrEmpty(roleStr))
                return Result<LoginResponse>.Failure(ErrorCode.AuthTokenInvalid, "令牌缺少必要的声明信息");
        ...
        {
            return Result<LoginResponse>.Failure(ErrorCode.AuthAccessTokenExpired, "令牌已过期，请重新登录");
        }
        catch (SecurityTokenException)
        {
            return Result<LoginResponse>.Failure(ErrorCode.AuthTokenInvalid, "无效的令牌");
        }
        catch
        {
