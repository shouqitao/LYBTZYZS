...
public class ToggleUserStatusCommandHandler
    : IRequestHandler<ToggleUserStatusCommand, Result<UserDetailDto>>
{
    ...
    )
    {
        ...
        if (user.IsSysAdmin)
            return Result<UserDetailDto>.Failure(
                ErrorCode.CannotDeleteSysAdmin,
                "系统管理员账号不可被禁用"
            );

        // UPDATEUSER-HIERARCHY-FIX: 层级校验（原 IsAdmin bool 粗粒度——Admin 可禁用 Admin 违反 USER-D05）
        ...
        var newStatus =
            user.Status == CommonStatus.Enabled ? CommonStatus.Disabled : CommonStatus.Enabled;

        // T5-1 #12 (US-REG-BR-006): 有待诊（Waiting）挂号的医生禁止禁用
        if (newStatus == CommonStatus.Disabled && user.Role == UserRole.Doctor)
        {
            var hasWaiting = await _registrationService.HasWaitingRegistrationsAsync(
                user.Id,
                cancellationToken
            );
            if (hasWaiting)
