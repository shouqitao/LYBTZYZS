        ...
        public async Task<ActionResult<ApiResponse>> Import([FromBody] List<HerbImportDto> dtos) {
            try {
                ...
                var createDtos = dtos.Select(dto => new HerbCreateDto {
                    ...
                    Price = dto.Price,
                    Effect = dto.Effect,
                    Usage = null, // HerbImportDto没有此字段
                    Stock = dto.Stock,
                    BatchNo = dto.BatchNo,
                    ExpireDate = dto.ExpireDate,
                    Status = LYBT.Shared.Models.Enums.CommonStatus.Enabled // HerbImportDto没有此字段，默认启用
                }).ToList();

                var result = await _herbService.ImportHerbsAsync(createDtos);
