...
namespace LYBT.WebAPI.Controllers
{
    ...
    public class MonitoringController : BaseSystemController
    {
        ...
        public IActionResult ReceiveAlertManagerWebhook([FromBody] AlertManagerWebhook webhook)
        {
            try
            {
                _logger.LogInformation("收到AlertManager告警通知: {@Webhook}", webhook);
            ...
            {
                _logger.LogError(ex, "处理AlertManager告警时发生错误");
        ...
        public IActionResult ReceiveCriticalAlert([FromBody] AlertManagerWebhook webhook)
        {
            try
            {
                _logger.LogCritical("收到严重告警通知: {@Webhook}", webhook);
            ...
            {
                _logger.LogError(ex, "处理严重告警时发生错误");
        ...
        public IActionResult ReceiveSecurityAlert([FromBody] AlertManagerWebhook webhook)
        {
            try
            {
                _logger.LogWarning("收到安全告警通知: {@Webhook}", webhook);
            ...
            {
                _logger.LogError(ex, "处理安全告警时发生错误");
        ...
        {
            _logger.LogWarning("处理告警: {AlertName}, 严重程度: {Severity}, 状态: {Status}",
        ...
        {
            _logger.LogCritical("严重告警处理: {AlertName}, 描述: {Description}",
                alert.Labels.GetValueOrDefault("alertname", "Unknown"),
        ...
        {
            _logger.LogWarning("安全告警处理: {AlertName}, 服务: {Service}",
                alert.Labels.GetValueOrDefault("alertname", "Unknown"),
