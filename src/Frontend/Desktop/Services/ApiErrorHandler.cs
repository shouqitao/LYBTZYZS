    ...
    public static class ApiErrorHandler
    {
        ...
        private static async Task<ServiceResult<T>> HandleApiExceptionAsync<T>(Refit.ApiException ex)
        {
            ...
            if (ex.HasContent)
            {
                ...
                {
                    // 尝试解析为ProblemDetails
                    var problemDetails = await ex.GetContentAsAsync<LYBT.WPF.Client.Core.Models.ProblemDetails>();
        ...
        private static Task<string> ExtractErrorMessageAsync<T>(Refit.ApiResponse<T> response)
        {
            if (response.Error != null)
            {
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(content))
                    {
                        var problemDetails = System.Text.Json.JsonSerializer.Deserialize<LYBT.WPF.Client.Core.Models.ProblemDetails>(content, new System.Text.Json.JsonSerializerOptions
                        {
