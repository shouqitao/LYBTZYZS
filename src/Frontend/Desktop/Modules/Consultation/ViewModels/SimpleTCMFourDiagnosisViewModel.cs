...
namespace LYBT.Desktop.Consultation.ViewModels
{
    ...
    public class SimpleTCMFourDiagnosisViewModel : BindableBase
    {
        ...
        private async Task SaveAsync()
        {
            try
            {
                ...
                if (result.IsSuccess)
                {
                    ...
                    // 发布保存成功事件
                    _eventAggregator.GetEvent<FourDiagnosisSavedEvent>().Publish(data);
        ...
        private async void OnImportHistoryData(ImportHistoryDataEventArgs args)
        {
            if (args.DataType != "FourDiagnosis" || 
                args.TargetStep != WorkflowStep.FourDiagnosis)
                return;
            ...
            try
            {
                ...
                // 加载历史四诊数据
                var result = await _consultationService.GetFourDiagnosisByMedicalCaseIdAsync(args.SourceMedicalCaseId);
                if (result.IsSuccess && result.Data != null)
                {
                    var data = result.Data;
                    ...
                    if (!string.IsNullOrWhiteSpace(Auscultation))
                        Auscultation += "\n";
                    Auscultation += data.Auscultation;
                    ...
                    if (!string.IsNullOrWhiteSpace(Palpation))
                        Palpation += "\n";
                    Palpation += data.Palpation;
                    
                    ImportSource = $"部分内容导入自历史就诊记录";
                    HasImportedData = true;
                    HasChanges = true;
                    
                    await _dialogService.ShowSuccessAsync("历史四诊信息导入成功", "成功");
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "导入历史四诊数据失败");
                await _dialogService.ShowErrorAsync("导入失败: " + ex.Message, "错误");
            }
            finally
            {
