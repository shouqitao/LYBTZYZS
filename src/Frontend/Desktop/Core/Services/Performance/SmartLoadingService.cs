...
namespace LYBT.Desktop.Core.Services.Performance
{
    ...
    public class SmartLoadingService : ISmartLoadingService
    {
        ...
        private readonly IPredictivePreloadService _predictiveService;
        ...
        public SmartLoadingService(
            ...
            IPredictivePreloadService predictiveService,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        // 记录用户行为
                        await _behaviorAnalyzer.RecordAccessAsync(resourceKey);
                        ...
                        // 触发预测性预加载
                        await TriggerPredictivePreloadAsync(resourceKey, options);
        ...
        public async Task SmartPreloadAsync(string currentContext, CancellationToken cancellationToken = default)
        {
            ...
            {
                // 获取预测的资源
                var predictedResources = await _predictiveService.PredictNextResourcesAsync(currentContext);
                
                if (!predictedResources.Any())
                {
                    return;
                }
                
                _logger.LogDebug("智能预加载 {Count} 个预测资源", predictedResources.Count());
                
                // 按预测概率排序
                var sortedResources = predictedResources
                    .OrderByDescending(r => r.Probability)
                    .Take(_config.MaxPreloadItems);
                
                // 异步预加载
                var preloadTasks = sortedResources.Select(resource =>
                    Task.Run(async () =>
                    {
                        try
                        {
                            // 动态调整优先级
                            var priority = resource.Probability > 0.8 ? LoadPriority.High :
                                         resource.Probability > 0.5 ? LoadPriority.Normal :
                                         LoadPriority.Low;
                            
                            await LoadAsync(
                                resource.ResourceKey,
                                ct => resource.LoadFunc(ct),
                                new LoadingOptions 
                                { 
                                    Priority = priority,
                        ...
                        {
                            _logger.LogWarning(ex, "智能预加载失败: {ResourceKey}", resource.ResourceKey);
                        }
                    }, cancellationToken));
        ...
        public async Task CleanupExpiredCacheAsync()
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    // 触发缓存压缩
                    _cache.Compact(0.1); // 移除10%最少使用的条目
                }
                catch (Exception ex)
                {
        ...
        private int CalculatePriority(string resourceKey, LoadingOptions options)
        {
            ...
            // 根据用户行为调整优先级
            var behaviorScore = _behaviorAnalyzer.GetResourceImportanceScore(resourceKey);
            var adjustedPriority = basePriority - (int)(behaviorScore * 10);
            
            return Math.Max(0, adjustedPriority);
        }

        private async Task TriggerPredictivePreloadAsync(string resourceKey, LoadingOptions options)
        {
            if (!options.EnablePredictivePreload)
            {
                return;
            }
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    await SmartPreloadAsync(resourceKey);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "触发预测性预加载失败");
                }
            });
        }
