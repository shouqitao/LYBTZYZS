...
public class RegistrationInputDtoValidator : AbstractValidator<RegistrationInputDto>
{
    public RegistrationInputDtoValidator()
    {
        ...
        RuleFor(x => x.PatientId).NotEmpty().WithMessage("患者不能为空");
        ...
        RuleFor(x => x.PatientName)
            ...
            .MaximumLength(100)
            .WithMessage("患者姓名长度不能超过100个字符");

        // 医生：必填（Guid.Empty 视为未提供）
        RuleFor(x => x.DoctorId).NotEmpty().WithMessage("医生不能为空");
        ...
        RuleFor(x => x.DoctorName)
            ...
            .MaximumLength(100)
            .WithMessage("医生姓名长度不能超过100个字符");

        // 挂号来源：枚举范围验证
        RuleFor(x => x.Source).IsInEnum().WithMessage("挂号来源值无效");

        // 挂号费：非负
        RuleFor(x => x.RegistrationFee).GreaterThanOrEqualTo(0).WithMessage("挂号费不能为负数");
