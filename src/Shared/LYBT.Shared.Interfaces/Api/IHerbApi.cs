    ...
    public interface IHerbApi
{
    ...
    Task<Refit.ApiResponse<HerbDto>> CreateHerbAsync([Refit.Body] HerbCreateDto request);

    /// <summary>
    /// 更新草药
    /// </summary>
    [Refit.Put("/api/v1/herbs/{id}")]
    Task<Refit.ApiResponse<HerbDto>> UpdateHerbAsync(Guid id, [Refit.Body] HerbUpdateDto request);

    /// <summary>
    /// 删除草药
    /// </summary>
    [Refit.Delete("/api/v1/herbs/{id}")]
    Task<Refit.ApiResponse<ApiResponse>> DeleteHerbAsync(Guid id);
}
}