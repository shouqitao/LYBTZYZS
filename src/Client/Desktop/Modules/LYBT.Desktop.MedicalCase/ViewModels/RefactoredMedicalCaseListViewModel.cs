    ...
    public class RefactoredMedicalCaseListViewModel : UnifiedViewModelBase
    {
        ...
        private async Task DeleteAsync(MedicalCaseDto? medicalCase)
        {
            ...
            try
            {
                ...
                var success = await _medicalCaseRepository.DeleteAsync(medicalCase.Id);
                if (success)
                {
        ...
        private async Task BatchDeleteAsync()
        {
            ...
            try
            {
                ...
                {
                    var success = await _medicalCaseRepository.DeleteAsync(id);
                    if (success)
                        successCount++;
                    else
                        failureCount++;
                ...
                {
                    await ShowErrorMessageAsync($"批量删除完成：成功 {successCount} 个，失败 {failureCount} 个");
                    await LoadDataAsync();
                }
            }
            catch (Exception ex)
