...
| **MedicalCaseCommandsViewModel** | `ChildViewModelBase`；9 个 `[RelayCommand]` **源生成命令**（`SaveAsync`/`SuspendAsync`/`CompleteAsync`/`PrintAsync`/`ExportPdfAsync`/`EnterEditMode`/`ImportFormula`/`CopyHistory`/`ClearHerbsAsync`；其中 5 个带 `CanExecute` 谓词）；数据经 `IMedicalCaseDataProvider` 从宿主 VM 获取，CanExecute 跨子 VM 边界需父 VM 手动 `RefreshCanExecute()`（宿主 `State` setter 调用，覆盖全部 6 个带谓词命令） | Save / Suspend / Complete / Print / ExportPdf / EnterEditMode / ImportFormula / CopyHistory / ClearHerbs |
