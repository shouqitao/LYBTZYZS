    ...
    IHerbApi herbApi) : IHerbQueryService
{
    ...
    public async Task<ServiceResult<PagedResult<HerbDto>>> GetPagedAsync(HerbPagedQueryDto query)
    {
        ...
        {
            _logger.LogDebug(
                "执行中药材分页查询，页码: {PageNumber}, 页大小: {PageSize}",
    ...
    public Task<ServiceResult<List<HerbDto>>> Search(string keyword)
    {
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            return Task.FromResult(ServiceResult<List<HerbDto>>.Failure("药材搜索失败"));
        }
    }
