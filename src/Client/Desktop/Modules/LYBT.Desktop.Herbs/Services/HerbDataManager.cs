    ...
    public class HerbDataManager : IHerbDataManager
    {
        ...
        public async Task InitializeAsync(Guid entityId)
        {
            try
            {
                ...
                if (herb != null) { _currentHerb = herb; _originalHerb = CloneHerb(herb); _logger.LogInformation("药材数据加载成功: HerbId={HerbId}", herb.Id); }
                else _logger.LogWarning("未找到药材数据: HerbId={HerbId}", entityId);
            }
            catch (Exception ex) { _logger.LogError(ex, "加载药材数据失败: HerbId={HerbId}", entityId); throw; }
        }

        public virtual async Task<bool> SaveAsync()
        {
            if (_currentHerb == null) { _logger.LogWarning("无法保存：当前药材数据为空"); return false; }
            try
            {
                _logger.LogInformation("开始保存药材数据: HerbName={HerbName}", _currentHerb.Name);
                var inputDto = ToInputDto(_currentHerb);
                var result = _currentHerb.Id == Guid.Empty ? await _herbRepository.CreateAsync(inputDto) : await _herbRepository.UpdateAsync(inputDto);
                if (result != null) { _currentHerb = result; _originalHerb = CloneHerb(result); _logger.LogInformation("药材数据保存成功"); return true; }
                return false;
            }
            catch (Exception ex) { _logger.LogError(ex, "保存药材数据失败: HerbName={HerbName}", _currentHerb?.Name); return false; }
        }

        public virtual async Task<bool> DeleteAsync()
        {
            if (_currentHerb == null || _currentHerb.Id == Guid.Empty) { _logger.LogWarning("无法删除：当前药材数据为空或ID无效"); return false; }
            try
            {
                _logger.LogInformation("开始删除药材: HerbId={HerbId}", _currentHerb.Id);
                await _herbRepository.DeleteAsync(_currentHerb.Id);
                _currentHerb = null; _originalHerb = null;
                _logger.LogInformation("药材删除成功"); return true;
            }
            catch (Exception ex) { _logger.LogError(ex, "删除药材失败: HerbId={HerbId}", _currentHerb?.Id); return false; }
        }
        ...
        {
            try { _logger.LogDebug("删除药材: HerbId={HerbId}", herbId); var result = await _herbRepository.DeleteAsync(herbId); _logger.LogInformation("药材删除成功: HerbId={HerbId}", herbId); return result; }
            catch (Exception ex) { _logger.LogError(ex, "删除药材失败: HerbId={HerbId}", herbId); throw; }
        }

        private bool IsHerbChanged() => _currentHerb != null && _originalHerb != null &&
            (_currentHerb.Name != _originalHerb.Name || _currentHerb.PinYinCode != _originalHerb.PinYinCode || _currentHerb.Origin != _originalHerb.Origin ||
             _currentHerb.Spec != _originalHerb.Spec || _currentHerb.Unit != _originalHerb.Unit || _currentHerb.Price != _originalHerb.Price ||
