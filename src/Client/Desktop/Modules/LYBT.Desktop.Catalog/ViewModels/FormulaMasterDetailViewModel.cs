...
namespace LYBT.Desktop.Catalog.ViewModels
{
    ...
    public partial class FormulaMasterDetailViewModel : MasterDetailViewModelBase<FormulaListDto, FormulaDetailModel>
    {
        ...
        private readonly IFormulaExcelService _formulaExcelService;
        ...
        /// <summary>批量导入重复处理策略选项（AC②：跳过/更新/报错；与列表的 DuplicateDosageStrategy 无关）</summary>
        ...
        [ObservableProperty]
        private LYBT.Shared.Models.Enums.DuplicateStrategy _importDuplicateStrategy = LYBT.Shared.Models.Enums.DuplicateStrategy.Skip;
        ...
        public FormulaMasterDetailViewModel(
            ...
            IFormulaExcelService formulaExcelService,
            ...
            : base(viewModelServices, masterDetailServices)
        {
            ...
            _formulaExcelService = formulaExcelService ?? throw new ArgumentNullException(nameof(formulaExcelService));
        ...
        /// <summary>
        /// 下载导入模板（US-FORM-013 / US-SHELL-021 AC①）：服务端 JSON 字段定义 → 本地 Excel 模板（.xlsx）。
        /// </summary>
        [RelayCommand]
        private async Task DownloadImportTemplateAsync()
        {
            try
            {
                ...
                var workbook = _formulaExcelService.GenerateTemplate(result.Data);
        ...
        /// <summary>
        /// 批量导入验方（US-FORM-006 / US-SHELL-021 AC③⑤）：
        ...
        private async Task ImportFormulasAsync()
        {
            ...
            try
            {
                ...
                var confirmed = await MasterDetailServices.Dialog.ShowConfirmAsync(
                    $"将导入 {request.Formulas.Count} 条验方记录（{ImportBatchRunner.CountBatches(request.Formulas.Count)} 批，重复策略：{DuplicateStrategyOptions.GetDisplay(ImportDuplicateStrategy)}），是否继续？",
                    "确认导入");
                if (!confirmed) return;

                var report = await ImportFormulasInBatchesAsync(request.Formulas, request.FileName);
                ImportReport = report;

                Logger.LogInformation("验方批量导入完成: 总 {Total} 行, 成功 {Success}, 失败 {Failure}, 跳过 {Skipped}, 批 {Batches}, 中止 {Aborted}",
        ...
        /// </summary>
        private async Task<ImportReport> ImportFormulasInBatchesAsync(IReadOnlyList<FormulaImportItemDto> rows, string? fileName)
        {
            var report = new ImportReport();
            var totalBatches = ImportBatchRunner.CountBatches(rows.Count);
            var completedBatches = 0;
            var matchedHerbs = 0;
            var unmatchedHerbs = 0;
            ...
            try
            {
                await ImportBatchRunner.RunAsync(
                    ...
                    async (batch, offset) =>
                    {
                        var result = await _formulaService.BatchImportAsync(new FormulaBatchImportInputDto
                        {
                            Formulas = batch,
                            FileName = fileName,
                        ...
                        matchedHerbs += data.MatchedHerbsCount;
                        unmatchedHerbs += data.UnmatchedHerbsCount;

                        foreach (var failure in data.Failures)
                        {
                            var reason = string.IsNullOrWhiteSpace(failure.ErrorDetails)
                                ? failure.ErrorMessage
                                : $"{failure.ErrorMessage}（{failure.ErrorDetails}）";
                            report.AddFailure(offset + failure.RowIndex, failure.FormulaName, reason);
                        }
            ...
            report.SetExtraSummary($"匹配药材 {matchedHerbs} 味，未匹配 {unmatchedHerbs} 味");
            return report;
        }

        /// <summary>导出验方数据（US-FORM-013 / US-SHELL-021 AC①）：服务端 JSON → 本地 Excel（.xlsx）。</summary>
        [RelayCommand]
        private async Task ExportFormulasAsync()
        {
            try
            {
                ...
                {
                    Filter = "Excel 文件|*.xlsx",
                    FileName = $"验方导出_{DateTime.Now:yyyyMMddHHmmss}.xlsx",
                    Title = "保存导出文件"
                };
                if (dialog.ShowDialog() != true) return;

                // AC①：列定义以服务端 export 为权威，仅表现层渲染为 .xlsx（后端契约不变）
                var workbook = _formulaExcelService.GenerateExportFile(result.Data);
                await File.WriteAllBytesAsync(dialog.FileName, workbook);
                await MasterDetailServices.Dialog.ShowSuccessAsync($"验方数据已导出到：{dialog.FileName}", "导出成功");
            }
            catch (Exception ex)
            {
