...
namespace LYBT.Desktop.Shared.ViewModels.Herbs
{
    /// <summary>
    /// 中药材管理视图模型
    /// </summary>
    public class HerbManagementViewModel : BaseServiceManagementViewModel<HerbDto>
    {
        ...
        private async Task ToggleHerbStatusAsync()
        {
            ...
            try
            {
                var statusText = SelectedHerb.Status == CommonStatus.Enabled ? "禁用" : "启用";
        ...
        private async Task ViewHerbAsync()
        {
            ...
            try
            {
                ...
                _dialogService.ShowDialog("HerbAddEditDialog", dialogParameters, (Action<IDialogResult>)null!);
            }
            catch (Exception ex)
            {
