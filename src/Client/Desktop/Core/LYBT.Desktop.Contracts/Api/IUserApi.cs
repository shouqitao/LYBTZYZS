...
namespace LYBT.Desktop.Contracts.Api
{
    ...
    internal interface IUserApi
    {
        ...
        Task<ApiResponse<UserDetailDto>> CreateUserAsync([Refit.Body] UserInputDto request, CancellationToken ct = default);

        /// <summary>
        /// 更新用户
        /// </summary>
        [Refit.Put("/api/v1/users/{id}")]
        Task<ApiResponse<UserDetailDto>> UpdateUserAsync(Guid id, [Refit.Body] UserInputDto request, CancellationToken ct = default);
        ...
        Task<ApiResponse<UserDetailDto>> ChangeProfileAsync(Guid id, [Refit.Body] ChangeProfileDto request, CancellationToken ct = default);

        /// <summary>
        /// 修改密码 (Issue #1887-1892)
        /// </summary>
        [Refit.Put("/api/v1/users/{id}/change-password")]
        Task<ApiResponse> ChangePasswordAsync(Guid id, [Refit.Body] ChangePasswordRequest request, CancellationToken ct = default);

        /// <summary>
        /// 管理员重置用户密码 (Issue #1910)
        /// </summary>
        [Refit.Post("/api/v1/users/{id}/reset-password")]
        Task<ApiResponse<ResetPasswordResponseDto>> ResetPasswordAsync(Guid id, [Refit.Body] ResetPasswordRequest request, CancellationToken ct = default);
