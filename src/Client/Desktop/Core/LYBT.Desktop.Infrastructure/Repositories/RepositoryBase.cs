...
namespace LYBT.Desktop.Infrastructure.Repositories
{
        ...
        where TUpdateDto : class
    {
        ...
        protected RepositoryBase(TApi api, ILogger logger) : base(logger)
        {
        ...
        public virtual async Task<TDetailDto?> GetByIdAsync(Guid id)
        {
            ...
            try
            {
                Logger.LogDebug("[REPO] {EntityType}.GetById started - Id={Id}", entityType, id);
                var response = await CallApiGetByIdAsync(id);
                if (response.Data == null)
                {
                    Logger.LogWarning("[REPO] {EntityType}.GetById → NotFound - Id={Id}", entityType, id);
                    throw new InvalidOperationException($"获取失败：ID {id} 的实体不存在");
                }
                Logger.LogDebug("[REPO] {EntityType}.GetById completed - Id={Id}", entityType, id);
                return response.Data;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "[REPO] {EntityType}.GetById failed - Id={Id}", entityType, id);
                throw;
            }
        }
        ...
        public virtual async Task<PagedResult<TListDto>> GetPagedAsync(int page = 1, int pageSize = 20, string? keyword = null)
        {
            ...
            try
            {
                Logger.LogDebug("[REPO] {EntityType}.GetPaged started - Page={Page} PageSize={PageSize} Keyword={Keyword}", entityType, page, pageSize, keyword);
                ...
                Logger.LogDebug("[REPO] {EntityType}.GetPaged completed - TotalCount={TotalCount}", entityType, result.TotalCount);
                return result;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "[REPO] {EntityType}.GetPaged failed - Page={Page} PageSize={PageSize} Keyword={Keyword}", entityType, page, pageSize, keyword);
                throw;
            }
        }
        ...
        public virtual async Task<TDetailDto> CreateAsync(TCreateDto dto)
        {
            ...
            try
            {
                Logger.LogInformation("[REPO] {EntityType}.Create started", entityType);
                var response = await CallApiCreateAsync(dto);

                if (!response.Success)
                {
                    Logger.LogWarning("[REPO] {EntityType}.Create → Failed - Message={Message}", entityType, response.Message);
                ...
                if (response.Data == null)
                {
                    Logger.LogWarning("[REPO] {EntityType}.Create → NoData", entityType);
                    throw new InvalidOperationException("创建失败：服务器未返回数据");
                }

                Logger.LogInformation("[REPO] {EntityType}.Create completed", entityType);
                return response.Data;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "[REPO] {EntityType}.Create failed", entityType);
                throw;
            }
        }
        ...
        public virtual async Task<TDetailDto> UpdateAsync(TUpdateDto dto)
        {
            ...
            {
                Logger.LogError("[REPO] {EntityType}.Update → InvalidId", entityType);
                throw new ArgumentException("更新DTO必须包含有效的ID", nameof(dto));
            }

            try
            {
                Logger.LogInformation("[REPO] {EntityType}.Update started - Id={Id}", entityType, id);
                var response = await CallApiUpdateAsync(id.Value, dto);

                if (!response.Success)
                {
                    Logger.LogWarning("[REPO] {EntityType}.Update → Failed - Id={Id} Message={Message}", entityType, id, response.Message);
                    // 直接传递服务器消息，避免重复前缀（上层会添加操作名称）
                ...
                if (response.Data == null)
                {
                    Logger.LogWarning("[REPO] {EntityType}.Update → NotFound - Id={Id}", entityType, id);
                    throw new InvalidOperationException($"更新失败：ID {id.Value} 的实体不存在");
                }

                Logger.LogInformation("[REPO] {EntityType}.Update completed - Id={Id}", entityType, id);
                return response.Data;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "[REPO] {EntityType}.Update failed - Id={Id}", entityType, id);
                throw;
            }
        }
        ...
        public virtual async Task<bool> DeleteAsync(Guid id)
        {
            ...
            try
            {
                Logger.LogInformation("[REPO] {EntityType}.Delete started - Id={Id}", entityType, id);
                var response = await CallApiDeleteAsync(id);
                if (response.Success)
                {
                    Logger.LogInformation("[REPO] {EntityType}.Delete completed - Id={Id}", entityType, id);
                }
                else
                {
                    Logger.LogWarning("[REPO] {EntityType}.Delete → Failed - Id={Id} Message={Message}", entityType, id, response.Message);
                }
                return response.Success;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "[REPO] {EntityType}.Delete failed - Id={Id}", entityType, id);
                return false;
            }
        }
        ...
        public virtual async Task<List<TListDto>> SearchAsync(string keyword)
        {
            ...
            try
            {
                Logger.LogDebug("[REPO] {EntityType}.Search started - Keyword={Keyword}", entityType, keyword);
                var response = await CallApiGetPagedAsync(1, 100, keyword);
                var items = response.Data?.Items ?? new List<TListDto>();
                Logger.LogDebug("[REPO] {EntityType}.Search completed - Count={Count}", entityType, items.Count);
                return items;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "[REPO] {EntityType}.Search failed - Keyword={Keyword}", entityType, keyword);
                throw;
            }
        }
