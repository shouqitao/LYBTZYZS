...
2. **强制架构文档阅读清单**：
   ...
   - [ ] docs/explanation/architecture/{server|client|shared}/README.md - 对应层架构指南
   ...
   - [ ] docs/explanation/architecture/client/shell-layer-design.md - Shell层设计（如存在）
   - [ ] docs/explanation/architecture/client/viewmodel-patterns.md - ViewModel模式（如存在）

   #### 跨端设计
   - [ ] docs/explanation/architecture/shared/README.md - 共享架构
   - [ ] docs/explanation/architecture/shared/dto-design.md - DTO设计标准（如存在）

   ### Level 3 - 专题架构（可选）
   - [ ] docs/explanation/architecture/decisions/ADR-*.md - 相关架构决策记录
   - [ ] docs/explanation/advanced-patterns.md - 高级模式（如涉及复杂重构）

   注意：部分详细架构文档可能不存在，需根据实际情况调整清单
   ```
...
2. **读取核心架构文档**：
   ...
   - docs/explanation/architecture/{server|client|shared}/README.md
3. **读取需求文档的架构约束章节**：
   - 确认聚合根约束
   - 确认三层架构参考
   - 确认技术黑名单
...
**文件路径**：`docs/explanation/{topic}-design.md`
...
def auto_validate_design():
    # 1. 读取生成的设计文档
    design_doc_path = "docs/explanation/{feature-name}-design.md"
...
✓ 写入设计文档：docs/explanation/medicalcase-consultation-prescription-refactoring-design.md
...
📊 生成统计：
- 设计文档：docs/explanation/medicalcase-consultation-prescription-refactoring-design.md
...
✓ 写入设计文档：docs/explanation/feature-xyz-design.md
